...
// 자평진전 성패 — 원문이 넷으로 가르므로 넷 다 나와야 한다 (2026-08-28 개정).
// 예전에는 「성격:파격 ≈ 66:66」이었는데, 그 기대값은 원문의 반쪽만 보던 이분법에서
// 나온 것이라 뜻이 없어졌다. 이제는 네 칸이 다 잡히는지와 그 비율만 본다.
const 판정수 = {};
let 상신있 = 0, 격n = 0;
for (let i = 0; i < 800; i++) {
  const y = 1960 + (i*7919)%60, m = 1 + (i*104729)%12, d = 1 + (i*1299709)%28, hh = (i*15485863)%24;
  let R; try { R = E.calc({year:y,month:m,day:d,hour:hh,minute:30,gender:'M',
    place:'KR:서울',longitude:126.98,tzOffset:null,solarCorrection:true}); } catch(e){ continue; }
  격n++;
  const J = T.gyeok(R);
  판정수[J.판정] = (판정수[J.판정] || 0) + 1;
  if (J.상신) 상신있++;
}
const 넷 = ['섰다','띠었다','구제됐다','깨졌다'];
const pc = k => (판정수[k] || 0) / 격n * 100;
ok('D','자평진전 성패 네 칸이 다 나온다', 넷.every(k => (판정수[k]||0) > 0),
   넷.map(k => k + ' ' + pc(k).toFixed(1) + '%').join(' · '));
ok('D','깨졌다 5~30% · 섰다 20~55%', pc('깨졌다')>=5 && pc('깨졌다')<=30 && pc('섰다')>=20 && pc('섰다')<=55,
   `깨졌다 ${pc('깨졌다').toFixed(1)}% · 섰다 ${pc('섰다').toFixed(1)}%`);
ok('D','상신을 지목한다 (70% 이상)', 상신있/격n >= 0.70,
   `${(상신있/격n*100).toFixed(1)}% — 격을 세우는 글자가 무엇인지 말해준다`);
...
// L22 국에 먹힌 지지는 제 십신으로 명령을 못 낸다 — 천간에 뜨면 발동한다
//   「진월 정화일간이 진토가 삼합되면 진토 상관명령을 잃는다.
//     천간에 무토가 오면 상관이 발동되는 것이고」
{
  const 없 = 기둥('壬申','甲辰','丁卯','庚子');   // 辰중 戊 가 국에 먹힌다. 천간에 토 없음
  const 있 = 기둥('壬申','戊辰','丁卯','庚子');   // 월간에 戊 투출 — 상관이 발동한다
  const a = T.gyeok({ pillars: 없, analysis: E.strengthOf(없) });
  const b = T.gyeok({ pillars: 있, analysis: E.strengthOf(있) });
  const 상관있 = (J) => (J.근거 && (J.근거.띠었다 || []).concat(J.근거.깨졌다 || [])
    .some(t => t.indexOf('상관') >= 0));
  ok('L', 'L22 국에 먹힌 지장간은 십신을 잃는다', !상관있(a) && 상관있(b),
     '천간에 토 없으면 ' + a.판정 + ' · 戊 투출하면 ' + b.판정 + ' (상관 발동)');
}
