...
(function (global) {
  ...
  function 범주(앞, 지금) {
    ...
    const 이름 = (g) => E.STEMS[g.stem] + '(' + g.십신 + ')';
    ...
    if (p.막힌극 && p.막힌극.length) {
      ...
      if (잃은.length) return { 범주: 범주표.약화, 근거: 잃은.map(이름).join('·') + '이 합거되고 ' + p.구응.map(이름).join('·') + '이 대신 막음 — 사장님 확인 대기(다른 글자가 대신 막는 것을 구응으로 치는가)' };
  ...
  function 요약(p) {
    return p.층들.map(층 => {
      const 격 = 층.격 ? (층.격.지금격 ? 층.격.지금격 + '격' : '격 없음') + (층.격.성패 ? '(' + 층.격.성패.판정 + ')' : '') : '';
