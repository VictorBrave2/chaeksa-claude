...
(function (global) {
  ...
  function 돌아감(t) {
    ...
    t.쌍.filter(r => r.관계 === '극' && r.from.산다 && r.to.산다 && !r.from.일간 && !r.to.일간 && !r.from.운 && !r.to.운).forEach(r => {
      if (r.힘차이 && !r.통관 && !r.제복) { 줄.push(이가(첫(r.from)) + ' ' + 을를(이름(r.to.stem)) + ' 극하려 하지만 힘이 세 배 넘게 모자라요. 극이 안 돼요.' + 뜻한번(r.from)); return; }
  ...
  function 운한줄(R, 원표0, u, 이름표, 앞운들) {
    ...
    t.쌍.filter(x => x.from === g && x.관계 === '극' && !x.to.일간 && x.to.산다 && g.산다).forEach(x => {
      if (x.힘차이 && !x.통관 && !x.제복) { 말.push(을를(이름(x.to.stem)) + ' 극하려 하지만 힘이 세 배 넘게 모자라요.'); return; }
