...
(function (global) {
  ...
  function 근거말(s) {
    ...
    s = s.replace(/^(\S+격) → (\S+격)(?: \(.*\))?$/, (m, a, b) => 이가(a) + ' ' + 조(b, '으로', '로') + ' 바뀌어요')   // 구조 전환 — 취격 근거(한자)는 위 변격 줄이 말한다
      ...
      .replace(/격신 힘이 (.+?)[을를] 두 배 넘게 눌러 못 침$/, (m, y) => '격신 힘이 ' + y + '의 두 배가 넘어서, ' + 이가(y) + ' 격신을 못 쳐요')
  ...
  function 운한줄(R, 원표0, u, 이름표, 앞운들, 층, 앞층) {
    ...
    const 말 = [];
    if (묶임.length) 말.push(이름표 + ' ' + 이가(묶어(묶임.map(x => 이름(x.stem)))) + ' 합거돼요. 사주의 흐름이 변질되었어요. 그동안 ' + 이가(묶어(묶임.map(x => 이름(x.stem)))) + ' 없는 거예요.');
    ...
    try {
      ...
      if (지금 && 지금.변질 && !(앞격 && 앞격.변질 && 앞격.지금격 === 지금.지금격)) {
        if (지금.지금격) 말.push(원격 + '격이 ' + 지금.지금격 + '격으로 변격했어요.' + (지금.주인 ? ' 이제 ' + 이가(이름(지금.주인.stem)) + ' 격신이에요.' : ''));   // 주인 없이 지장간 본기로 잡힌 격이면 주인이 null 이다
        else 말.push(원격 + '격의 격신이 합거됐어요. 격을 잡을 글자가 없어요.');
      }
      ...
      if (지금 && 지금.성패 && 판(지금) !== 판(앞격)) {
        ...
        const 첫근거 = (arr) => (arr && arr.length) ? String(Gk && Gk.공주님말of ? Gk.공주님말of(arr[0]) : arr[0]).replace(/\s*\(\d+조\)/g, '').replace(/\.$/, '') + '.' : '';
        if (j.판정 === '깨졌다') {
    ...
    if (층 && 층.범주) { const 근 = 근거말(층.근거); 말.push('판정은 「' + 층.범주 + '」' + (근 ? ' — ' + 근 : '') + '.'); }
    return 말.join(' ');
  }
  ...
  function 조심(걸린) {
    ...
    out.push(이가(이름(합짝)) + ' 오면 ' + 이가(이름(걸린.stem)) + ' 합거돼요.');
  ...
  function render(R, today, box, opts) {
    ...
    // ② 돌아감
    const d = 돌아감(원표);
    const 걸린말 = d.걸린 ? '<p class="wg-key">그래서 이 사주는 <b>' + esc(이름(d.걸린.stem)) + '</b> 하나에 걸려 있어요. ' + esc(이가(이름(d.걸린.stem))) + ' 힘이 있으면 통관·제복이 서고, 합거되면 극이 바로 들어와요.</p>' : '';
    // ③ 격
    // 취격 근거(「여기 乙의 오행이 투출」)의 맨 한자는 이름(「을목(乙)」)으로 바꿔 낸다 — 한자만 따로 내지 않는다(머리 주석).
    const 격근거 = String(원표.격 && 원표.격.근거 || '월지가 정한 격').replace(/[甲乙丙丁戊己庚辛壬癸]/g, (c) => 이름(E.STEMS.indexOf(c)));
    const 격 = 원표.격 ? '<p class="wg-gk top"><b>' + esc(원표.격.이름) + '격</b> — ' + esc(조(격근거, '이에요', '예요')) + '.' + (원표.격.상신 ? ' 상신은 ' + esc(원표.격.상신) + '이에요.' : '') + '</p>' : '';
    // ④⑤ 운
    ...
    const 운줄 = [
      대운 ? '<p class="wg-un">' + esc(운한줄(R, 원표, 대운, 간지(대운) + ' 대운, 이 10년 동안', [], 층of('대운'), 앞of('대운'))) + '</p>' : '',
      올해 ? '<p class="wg-un">' + esc(운한줄(R, 원표, 올해, 간지(올해) + ' 올해, 이 한 해', [대운], 층of('올해'), 앞of('올해'))) + '</p>' : '',
      이달 ? '<p class="wg-un">' + esc(운한줄(R, 원표, 이달, 간지(이달) + ' 이달, 이 한 달', [대운, 올해], 층of('이달'), 앞of('이달'))) + '</p>' : '',
