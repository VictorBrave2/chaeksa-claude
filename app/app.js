    ...
    try {
      ...
      try {
        ...
        원.성립.filter(x => !운성.has(키(x))).forEach(x => {
          줄.push('타고난 기운에서 ' + 자리말(x) + (x.종류 === '충' ? '가 서로 부딪히던 것이 오늘은 묶여 잠잠해진다고 봅니다' : '가 붙어 있던 것이 오늘은 풀린다고 봅니다') + ' — ' + x.글자 + ' ' + x.종류 + '.');
        });
        // 원국에서 묶여 있다가 살아난 것(오늘 글자가 낀 것은 위에서 이미 말했다)
        운.성립.filter(x => !원성.has(키(x)) && !(x.자리 || []).includes('오늘')).forEach(x => {
          줄.push('타고난 기운에서 묶여 있던 ' + 자리말(x) + (x.종류 === '충' ? '의 부딪힘이 오늘 살아난다고 봅니다' : '의 붙음이 오늘 선다고 봅니다') + ' — ' + x.글자 + ' ' + x.종류 + '.');
        });
