  ...
  function 일일리포트(d) {
    ...
    try {
      ...
      try {
        ...
        const 자리말 = (x) => (x.자리 || []).filter(a => a !== '오늘').map(a => (자리이름[a] || a) + '(' + a + ')').join('·');
        ...
        운.성립.filter(x => (x.자리 || []).includes('오늘')).forEach(x => {
          줄.push('오늘 글자 ' + 지 + 토(지독, '이', '가') + ' 공주님 ' + 자리말(x) + '와 ' + (x.종류 === '충' ? '부딪힙니다' : '붙습니다') + ' — ' + x.글자 + ' ' + x.종류 + '. 이런 날은 그 자리의 일이 움직인다고 봅니다.');
        });
        // 원국에 있던 것이 뺏겨 풀린 것
        원.성립.filter(x => !운성.has(키(x))).forEach(x => {
          줄.push('타고난 판에서 ' + 자리말(x) + (x.종류 === '충' ? '가 서로 부딪히던 것이 오늘은 묶여 잠잠해진다고 봅니다' : '가 붙어 있던 것이 오늘은 풀린다고 봅니다') + ' — ' + x.글자 + ' ' + x.종류 + '.');
        });
        // 원국에서 묶여 있다가 살아난 것(오늘 글자가 낀 것은 위에서 이미 말했다)
        운.성립.filter(x => !원성.has(키(x)) && !(x.자리 || []).includes('오늘')).forEach(x => {
          줄.push('타고난 판에서 묶여 있던 ' + 자리말(x) + (x.종류 === '충' ? '의 부딪힘이 오늘 살아난다고 봅니다' : '의 붙음이 오늘 선다고 봅니다') + ' — ' + x.글자 + ' ' + x.종류 + '.');
        });
        ...
        [['year', '연간'], ['month', '월간'], ['hour', '시간']].forEach(([k, 이름]) => {
          ...
          줄.push('오늘 하늘 글자 ' + 글 + 토(글독, '이', '가') + ' 타고난 판의 ' + 원글 + '(' + 이름 + ')' + 토(원독, '과', '와') + ' 합합니다. ' + (둘째 ? '대운이나 이달에 같은 글자가 이미 떠 있어 둘째 글자라, 묶이지 않고 뚫린다고 봅니다.' : '첫 번이라 오늘은 묶인다고 봅니다.'));
        });
        out.엮임 = 줄.length ? 줄.join(' ') : '오늘 글자는 타고난 판의 얽힘을 바꾸지 않는다고 봅니다.';
      } catch (e) { out.엮임 = ''; }
    } catch (e) {}
    return out;
  }
