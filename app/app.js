...
(function () {
  ...
  function renderChong() {
    ...
    // 문진감의 기준(2026-08-29 교정): 「본인이 체감으로 맞다/아니다 할 수 있는 결론」만.
    // 구조 진단문(회전문·채가는 자리 같은 메커니즘 설명)은 우리 용어라 문진이 아니라
    // 근거 자리로 간다. 문장은 승인된 법(제19조 사분면)의 결론 문구에서만 꺼낸다.
    (function () {
      const 남 = ((R.input && R.input.gender) || profile.gender || 'M') === 'M';
      const 양간 = R.pillars.day.stem % 2 === 0;
      const 접착 = (남 && 양간) || (!남 && !양간);
      사람문.push(접착
        ? { 문장: '한번 마음이 붙으면 놓기보다 붙드는 쪽입니다 — 인연을 직접 잡는 접착을 타고났습니다.',
            근거: (LW && LW.말 && LW.말[0]) || '배우자성이 오면 일간이 직접 합합니다 — 일간합 접착 구조.' }
        : { 문장: '인연이 시작되지 않아서가 아니라, 시작된 인연이 오래 머물지 않는 일이 반복돼 온 쪽입니다.',
            근거: (LW && LW.말 && LW.말[0]) || '배우자성과 일간이 합하지 않는 구조 — 붙드는 접착을 타고나지 않았습니다.' });
    })();
    // 재물은 체감 문장만 고른다 — 「재성이 지지 속에 숨어 있다」 같은 차트 사실은 배제
    const 체감재물 = (MW && MW.말 ? MW.말 : []).find(t => t.length >= 45 && t.indexOf('글자') < 0 && t.indexOf('하늘') < 0 && t.indexOf('지지') < 0);
    if (체감재물) 사람문.push({ 문장: 체감재물, 근거: (MW && MW.말 && MW.말[0]) || '재물 구조 진단 — 원국의 글자 배치에서.' });
          ...
          if (nx && paid && T.whoLovesMe) {
            ...
            nx.innerHTML = `<div class="paidbox"><p class="pb-k">결제 열람 — 어떤 사람이 당신을 사랑하는가</p>
              ${w.진사랑 ? `<div class="pb-verdict"><p class="pb-k" style="margin-bottom:8px">진정한 사랑 — 배우자 방에 앉은 글자</p>
                <p class="pb-vd">◆ 당신의 배우자 방 ${esc(w.진사랑.궁)}에 앉아 있는 글자는 <b>${esc(w.진사랑.글자)}</b> — 진정한 사랑은 <b>${esc(w.진사랑.기운말)}을 짙게 지닌 사람</b>이기 쉽습니다.</p>
                <p class="pb-vd">◆ ${esc(w.진사랑.기운풀이)}</p>
                <p class="pb-vd">◆ ${esc(w.진사랑.인물)}</p>
                <p class="pb-vd">◆ 당신에게는 ${esc(w.진사랑.십신)}의 자리 — ${esc(w.진사랑.십신뜻)}</p>
                ${w.진사랑.회전문
                  ...
                  : `<p class="pb-vd">◆ 이 글자를 합으로 데려오는 방아쇠는 <b>${esc(w.진사랑.방아쇠글자)}</b>${w.진사랑.맞물림 ? ' — 당신의 배우자성이기도 합니다. 궁과 성이 맞물린 사주라 이 사슬이 두 겹으로 조입니다' : ''}.${w.진사랑.해들.length ? ` <b>${w.진사랑.해들.join('·')}년</b>에 그 사람이 방으로 들어오기 쉽습니다.` : ''}</p>`}
              </div>` : ''}
            ...
            aiNarrate(nx, 'whom', {
              ...
              진정한사랑: w.진사랑 ? { 배우자방: w.진사랑.궁, 앉은글자: w.진사랑.글자,
                ...
                방아쇠: w.진사랑.방아쇠글자 + (w.진사랑.맞물림 ? ' (당신의 배우자성이기도 — 궁과 성이 맞물림)' : ''),
                들어오는해: w.진사랑.해들 } : null,
