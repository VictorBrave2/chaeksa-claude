...
(function (global) {
  ...
  function 층격(pillars, 운들, 원격, R, 계절지지) {
    ...
    const 성패 = (격, 주인) => {
      ...
      const 인성산다 = t.글자.some(g => !g.일간 && g.산다 && (g.십신 === '정인' || g.십신 === '편인'));
      if (격 === '정관' && 주인 && 주인.발동 && 인성산다) 침 = 침.filter(c => !(c.십신 === '식신' || c.십신 === '상관'));
      // 71조 ⑤ — 살이 합으로 묶이면 관이 맑아진다. 맑아진 관은 발동한 글자다(甲 묶임 → 乙). 「관이 투출했는데 합거된다」(기신)를 걸지 않는다.
      if (j && 격 === '정관' && 주인 && 주인.발동 && 주인.묶인 && 주인.묶인.십신 === '편관') {
        const 근 = Object.assign({}, j.근거 || {});
        근.띠었다 = (근.띠었다 || []).filter(x => x !== '관이 투출했는데 합거된다');
        근.섰다 = (근.섰다 || []).concat([이가('살 ' + 이름(주인.묶인.stem)) + ' 합으로 묶여 관이 맑아졌다 — 맑아진 관은 ' + 이름(주인.stem)]);
        let 판 = j.판정;
        if (판 === '띠었다' && !근.띠었다.length) 판 = (근.구제 && 근.구제.length) ? '구제됐다' : '섰다';
        j = Object.assign({}, j, { 근거: 근, 판정: 판, ok: 판 === '깨졌다' ? 0 : j.ok });
      }
    ...
    return { 원격, 지금격, 변질, 주인, 근거, 성패: 지금격 ? 성패(지금격, 주인) : null, 묶인주인, 발동 };
  }
