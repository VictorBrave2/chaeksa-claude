...
(function (global) {
  ...
  function 표(pillars, 운천간들, R, 계절지지) {
    ...
    for (const a of 글자) for (const b of 글자) {
      ...
      if (관계 === '극') {
        ...
        // 36조 3배 규칙 — 맞는 쪽은 같은 오행의 살아 있는 글자 힘을 합쳐 잰다(사장님 09-14 「화금으로 재가 둘이면 비겁 하나가 다 못 친다 — 맞는 말」).
        const b합 = 글자.filter(c => !c.일간 && c.산다 && c.오행 === b.오행).reduce((s, c) => s + (c.힘 || 0), 0) || b.힘;
        row.힘차이 = (!b.일간 && a.힘 != null && b.힘 != null && a.힘 > 0 && b합 >= 3 * a.힘);
  ...
  const 눈금 = {
    ...
    비율: { 최소: 0, 최대: null, 문턱: { 못침: 3 }, 말: '맞는 쪽 합산 ÷ 치는 쪽 · 3배부터 못 친다' },
  };
