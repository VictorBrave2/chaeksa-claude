...
(function (global) {
  ...
  function 층격(pillars, 운들, 원격, R, 계절지지) {
    ...
    // 71조 ⑥(09-24) 관살혼잡 — 한 층의 살아 있는 천간(합화한 글자 포함 · 발동 글자 포함)에 정관과 편관이 같이 있으면 관이 둘로 갈린다. 사례: 己巳 대운 壬子 세운, 丁壬 화목 → 丁 정관 · 壬 편관.
    const 산관 = t.글자.filter(g => !g.일간 && g.산다 && (g.십신 === '정관' || g.십신 === '편관')).concat(주인 && 주인.발동 ? [주인] : []);
    if (산관.some(g => g.십신 === '정관') && 산관.some(g => g.십신 === '편관'))
      변화.push({ 무엇: '관', 전: 원격, 후: '관살혼잡', 까닭: 산관.map(g => g.이름 + '(' + g.글자 + ') ' + g.십신 + (g.화 ? '(합화)' : '')).join(' · ') });
