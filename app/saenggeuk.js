...
(function (global) {
  ...
  function 글자들(pillars, 운천간들) {
    ...
    (운천간들 || []).forEach(u => {
      // 뒤에 온 운 글자는 원국 글자뿐 아니라 앞서 온 운 글자도 묶는다 — 월운 辛이 세운 丙을 묶으면 구응이 사라진다(09-14 사장님). 원국이 먼저, 그다음 앞 운.
      const 후보 = out.filter(x => !x.일간 && !x.합거 && E.isHap(x.stem, u.stem)).sort((a, b) => (a.운 ? 1 : 0) - (b.운 ? 1 : 0));
  ...
  function 표(pillars, 운천간들, R) {
    ...
    for (const a of 글자) for (const b of 글자) {
      ...
      if (관계 === '극') {
        ...
        // 맞는 쪽이 치는 쪽보다 세 배 이상 세면 못 친다(36조, 09-14 「3배 이상 힘이 차이난다면 경금이 갑목을 치기 어려운 것도 맞지?」). 일간은 힘을 안 재니 뺀다.
        row.힘차이 = (!b.일간 && a.힘 != null && b.힘 != null && a.힘 > 0 && b.힘 >= 3 * a.힘);
