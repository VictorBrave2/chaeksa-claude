...
(function (global) {
  ...
  function 칸그리기(v0, 속나, 속그) {
    ...
    if (v.사이 && v.사이.length) out.push('<h3>두 분 사이</h3>' + 문단(v.사이));
  ...
  function 그리기(box, 입력나, 입력그) {
    ...
    안전(1, '두 분의 여덟 글자', '나란히 놓고, 이 글이 무엇을 하는 것인지', () => {
      const v = Q.장1(C);
      return 칸그리기(v, 명식표(C.나.R) + '<h3>오행이 몇 글자씩</h3>' + 오행줄(v.오행.나), 명식표(C.그.R) + '<h3>오행이 몇 글자씩</h3>' + 오행줄(v.오행.그));
    });
    안전(2, '두 분의 일간', '각자를 가리키는 글자 둘이 만나면', () => 칸그리기(Q.장2(C)));
    안전(3, '배우자 자리를 맞대 보기', '두 분의 일지가 붙나 부딪히나', () => 칸그리기(Q.장3(C)));
    안전(4, '각자의 배우자 자리', '그 칸에 무엇이 앉았나', () => 칸그리기(Q.장4(C)));
    안전(5, '배우자별', '짝을 가리키는 글자가 어떤 모습인가', () => 칸그리기(Q.장5(C)));
    안전(6, '계절', '서로가 찾는 글자를 상대가 가졌나', () => 칸그리기(Q.장6(C)));
    안전(7, '그 사람 글자가 내 판에 들어오면', '격이 어떻게 되나', () => 칸그리기(Q.장7(C)));
    안전(8, '신살과 귀인', '각자의 신살 · 귀인, 상대 글자가 나에게 무슨 이름인가', () => {
      const v = Q.장8(C);
    ...
    안전(9, '그 사람 안의 나, 내 안의 그 사람', '서로를 가리키는 글자의 모습', () => 칸그리기(Q.장9(C)));
    ...
    안전(12, '앞으로 다섯 해', '같은 해에 두 분에게 오는 것', () => {
      const v = Q.장12(C);
      return 문단(v.머리) + v.해들.map(y => '<div class="jt-du"><div class="jt-du-h"><b>' + y.해 + '년</b><span>' + esc(y.간지) + '</span></div>'
