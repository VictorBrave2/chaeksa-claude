...
(function (global) {
  ...
  function 그리기(box, a, b, opts) {
    ...
    const 대화html = (arr) => arr.map(([누가, 말], i) => { const m = esc(말).replace(/\*\*(.+?)\*\*/g, '<b>$1</b>');
      ...
      if (누가 === '단서') { const 앞 = i > 0 && arr[i - 1][0] === '단서', 뒤 = i + 1 < arr.length && arr[i + 1][0] === '단서';   // 이어진 단서 줄은 한 상자로
        return (앞 ? '' : '<p class="ss-dan"><b>사주에서 본 근거</b>') + '<br>' + m + (뒤 ? '' : '</p>'); }
    ...
    const 알기칸 = (번, R, 주, 머리) => {
      ...
      const 주는 = !언.length ? { 제목: '좋아해도 행동으로는 잘 안 드러나요', 줄: 생.없음 || [] }
        : 조각.every(Boolean) ? { 제목: 조각.map(x => x.제목).join(' · '), 줄: [].concat(...조각.map(x => x.줄), (언.some(x => x.드러남) ? 생.드러남 : 생.숨음) || []) } : null;
      const 속 = '<h4 class="ss-sub">' + 누가 + ' 끌리는 사람</h4>' + (바 ? '<p class="ss-lead">' + esc(바.제목) + '</p>' + 칸(바꿔(바.줄, { '{주}': 주 })) : '')
        + '<h4 class="ss-sub">' + (주 === '당신' ? '내가' : '그 사람이') + ' 좋아할 때 하는 행동</h4>' + (주는 ? '<p class="ss-lead">' + esc(주는.제목) + '</p>' + 칸(바꿔(주는.줄, { '{주}': 주 })) : '');
    ...
    const 닿칸 = (받R, 줌R, 받, 주는이) => { const k = S.닿음키(받R, 줌R), d = 닿[k];
      const 머리 = (받 === '당신' ? '내가 바라는 걸 그 사람이 주나요?' : '그 사람이 바라는 걸 내가 주나요?');
      return '<div class="ss-daeum"><p class="ss-ask">' + esc(머리) + (d ? ' — <b>' + esc(d.말) + '</b>' : '') + '</p>' + (d ? 칸(바꿔(d.줄, { '{받}': 받, '{줌}': 주는이 })) : '') + '</div>'; };
    ...
    } else 때칸 = '<p class="ss-why">두 사람이 처음 만난 달을 넣으면, 그때와 지금 끌리는 사람과 좋아할 때 하는 행동이 어떻게 달라졌는지 보여 드려요.</p>';
    ...
    const 보이기 = (i, r, 적) => {
      ...
      d.querySelector('.ss-next').innerHTML = 글 ? 칸2(글) : '';
      if (적) S.적기(z.키, i, r);
    };
  ...
  function 대본(a, b, opts) {
    ...
    const 알기 = (R, 주) => { const 일주 = S.일주(R), 언 = S.언행(R), 바 = 바람[일주], 조각 = 언.map(x => 줌[x.키]).filter(Boolean);
      const 줄들 = [];
      줄들.push({ 누가: '책사', 말: (주 === '당신' ? '먼저 당신이 끌리는 사람이에요.' : '이번엔 그 사람이 끌리는 사람이에요.') });
      if (바) 줄로(바.줄, { '{주}': 주 }).forEach(x => 줄들.push(x)); else { /* 09-27 없는 일주 글은 안 낸다 */ }
      줄들.push({ 누가: '책사', 말: (주 === '당신' ? '당신은 좋아하면 이렇게 해요.' : '그 사람은 좋아하면 이렇게 해요.') });
    ...
    const 닿줄 = []; [[나R, 그R, '당신', '그 사람', '당신이 바라는 걸 그 사람이 주나요'], [그R, 나R, '그 사람', '당신', '그 사람이 바라는 걸 당신이 주나요']].forEach(([받R, 줌R, 받, 주는이, 머리]) => {
      const d = 닿[S.닿음키(받R, 줌R)];
      if (d) 닿줄.push({ 누가: '책사', 말: 머리 + '? — **' + d.말 + '**.' });
    ...
    } else 때줄.push({ 누가: '책사', 말: '두 사람이 처음 만난 달을 넣으면, 그때와 지금이 어떻게 달라졌는지 들려 드려요.' });
    // 09-25 v2(사장님 「4화를 제외하는 방안」): 「처음 만났을 때와 지금」 장은 넣지 않는다 — 때의 변질은 안정기 3 · 흔들림 1 사건이 한다
    const 맞줄 = 맞 ? [{ 누가: '책사', 말: '**안고 갈 것**이에요. 쉽게 안 바뀌어요.' }].concat(맞.안고.map(t => ({ 누가: '책사', 말: t })), [{ 누가: '책사', 말: '**맞춰 갈 것**이에요. 말 한마디로 달라져요.' }], 맞.맞춰.map(t => ({ 누가: '책사', 말: t })), [{ 누가: '책사', 말: '안고 갈지, 못 안고 갈지는 두 사람이 정해요.' }])
      : [];   // 09-27 GPT 지적 2: 「쓰고 있어요」 안 냄
    ...
    if (단계 === '썸') ((원고 || (대화 && 대화.썸)) ? S.질문 : []).forEach((q, i) => 장들.push({ 제목: q, 배경: (곳배경.썸 && 곳배경.썸[i]) || [배경[i % 배경.length]], 줄: 썸글(i) ? 대화줄(썸글(i)) : 줄로(원고[i]), 반응: null }));
      ...
      칸.질문.forEach((q, i) => 장들.push({ 제목: q, 배경: (곳배경[단계] && 곳배경[단계][i]) || [배경[i % 배경.length]], 줄: 단조(i) ? 대화줄(단조(i)) : 글들[i] ? 줄로(글들[i]) : [] })); }
