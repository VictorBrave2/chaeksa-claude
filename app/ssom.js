...
(function (global) {
  ...
  function 그리기(box, a, b, opts) {
    ...
    const 알기칸 = (번, R, 주, 머리) => {
      ...
      const 속 = '<h4 class="ss-sub">' + 누가 + ' 바라는 사람</h4>' + (바 ? '<p class="ss-lead">' + esc(바.제목) + '</p>' + 칸(바꿔(바.줄, { '{주}': 주 })) : '')
        + '<h4 class="ss-sub">' + 누가 + ' 마음을 보이는 법</h4>' + (주는 ? '<p class="ss-lead">' + esc(주는.제목) + '</p>' + 칸(바꿔(주는.줄, { '{주}': 주 })) : '');
