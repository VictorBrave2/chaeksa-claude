...
// 출산택일 신청 창구(2026-09-15 사장님 「출산택일 자체가 토스결제해야 넘어가지잖아」). 결제창이 열리기 전(토스 심사 중)에는
// 신청서를 네이버폼으로 받고 값 안내는 답장 메일로 한다. 심사가 끝나 라이브 키가 들어가면 이 줄을 지운다(빈 값이면 결제 흐름).
...
/* 회의 장면(council-<계절>[-벌].webp)이 계절마다 몇 벌인가. **있는 것만 적는다.**
 * 비어 있으면 랜딩이 그림을 부르지 않고 글자 첫 화면을 쓴다(app.js 회의장면).
 * 2026-09-22 — 09-12 밤에 그림을 지웠는데 여기 「계절마다 세 벌」이 남아 첫 화면마다 없는 그림을 세 번 찾았다(404). 비웠다.
 * 새 회의 장면이 들어오면 그 계절 숫자만 적으면 된다 — 그림을 실제로 받은 뒤에만 장면을 깐다. */
window.CHAEKSA_COUNCIL_VAR = {};
