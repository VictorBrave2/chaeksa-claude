...
(function (global) {
  ...
  function 연애해점수(R, y, 오행, 이름) {
    ...
    if (뺏는짝 !== undefined && E.STEM_ELEM[tf.year.stem] !== 오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + '이 공주님의 ' + 이름 + ' ' + E.STEMS[뺏는짝] + '을 합으로 데려가는 해 — 삼각·이탈 조심');
    }
  ...
  function 연애서술(d, db, 이름) {
    ...
    if (d.관계 === '육합') 문.push('그리고 그해의 ' + 지말(d.세지) + 이가(E.BRANCHES_KO[d.세지]) + ' 공주님의 배우자 자리 ' + 지말(db) + 을를(E.BRANCHES_KO[db]) + ' 끌어안는 합이었습니다 — 마음이 기울고 곁을 내주게 되는 결입니다.');
    else if (d.관계 === '삼합') 문.push('그해의 ' + 지말(d.세지) + '는 공주님의 배우자 자리 ' + 지말(db) + '와 삼합으로 맞물립니다 — 같이 다니고 같이 겪으며 가까워지는 결입니다.');
    ...
    if (d.뺏김) 문.push('조심할 해였습니다 — 그해의 ' + 간말(d.뺏김.손) + 이가(E.STEMS_KO[d.뺏김.손]) + ' 공주님의 ' + 간말(d.뺏김.성) + '을 합으로 데려가는 자리라, 곁의 인연이 흔들리거나 셋이 되기 쉬웠습니다.');
  ...
  function 재물해점수(R, y) {
    ...
    if (뺏긴재 !== undefined && E.STEM_ELEM[tf.year.stem] !== 재오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + '이 공주님의 재성 ' + E.STEMS[뺏긴재] + '을 합으로 데려가는 해 — 동업·보증·빌려주기 금물');
    }
  ...
  function whoLovesMe(R) {
    ...
    const 위치문 = (st) => {
      const 투 = [['year', p.year.stem], ['month', p.month.stem]].concat(p.hour ? [['hour', p.hour.stem]] : [])
        .find(x => x[1] === st);
      if (투) return '이 글자는 이미 공주님 원국 ' + 자리이름[투[0]] + '에 떠 있습니다 — 이 결의 사람이 벌써 삶에 들어와 있거나, 유난히 자주 만나집니다.';
      const 숨 = 지들0.find(b => (E.HIDDEN[b] || []).indexOf(st) >= 0);
      if (숨 !== undefined) return '이 글자는 공주님 지지 ' + E.BRANCHES[숨] + '(' + E.BRANCHES_KO[숨] + ') 속에 숨어 있습니다 — 가까운 자리에 이미 있는데 아직 알아보지 못했기 쉽습니다.';
      return '이 글자는 공주님 원국에 없습니다 — 운이 데려오는 사람입니다. 이 글자가 하늘에 뜨는 해에 만나기 쉽고, 그 해가 연애 스토리의 「열리는 해」와 같은 잣대로 짚은 자리입니다.';
    };
    ...
    const 사람 = (st, 종류) => {
      ...
      return {
        ...
        합: st === 합간,
        가로채임: 채는손 !== undefined
          ? '다만 이 글자로 오는 인연은 공주님 원국의 ' + E.STEMS[채는손] + '(' + E.STEMS_KO[채는손] + ')이 합으로 먼저 채가기 쉽습니다 — 스치기 쉬운 통로라, 이 사람일수록 빨리 잡아야 합니다.'
          : null,
      };
    };
    ...
    for (let y = 태어난해 + 17; y < 올해; y++) {
      ...
      if (ys === 궁본기) {
        ...
        사건들.push({ 해: y, 나이: a, 점수: 합됨 ? 9 : 8, 축: '관계',
          ...
          근거: '배우자 자리 속에만 있던 글자 ' + E.STEMS[ys] + '이 이 해 하늘로 나왔습니다' + (합됨 ? ' — 그리고 공주님 원국의 글자가 바로 합으로 잡았습니다' : '') + '.' });
      }
  ...
  function inyeonWhy(R) {
    ...
    // 유도·전수 검증: 열 일간 예외 0 (docs/19_법전.md 제19조).
    {
      ...
      if (접착) {
        const 합상대 = (ds + 5) % 10;
        사분면.push('먼저 구조 하나 — 공주님은 ' + (남 ? '양(陽)의 기운을 타고난 남성' : '음(陰)의 기운을 타고난 여성') + '입니다. '
          + (남 ? '아내의 글자 ' : '남편의 글자 ') + E.STEMS[합상대] + '(' + E.STEMS_KO[합상대]
          + ')이 오면 공주님의 일간이 먼저 손을 내밀어 맞잡습니다 — 인연을 제 손으로 붙드는 힘을 타고난 쪽입니다.');
        ...
        const 신 = (st) => E.TEN_GODS[E.tenGod(ds, st)];
        사분면.push('먼저 구조 하나 — 공주님은 ' + (남 ? '음(陰)의 기운을 타고난 남성' : '양(陽)의 기운을 타고난 여성') + '입니다. '
          ...
          + ')이 채가는 자리가 미리 예약되어 있습니다 — 인연이 오래 머물기 어렵던 구조적 이유입니다. 그래서 공주님께는 회전문을 피하는 글자와, 두 번 겹치는 때를 아는 것이 남보다 중요합니다.');
      }
      말 = 사분면.concat(말);
    }
  ...
  function monthWhy(R) {
    ...
    else if (a.strength === '신강') 말.push('공주님 사주는 힘이 안에 고이는 쪽입니다. 내보낼 자리가 있는 날에 움직여야 풀리고, 고이는 날에 웅크리면 답답해집니다 — 그 날들을 갈라 드립니다.');
    else 말.push('공주님 사주는 치우침이 적어 날의 결을 그대로 탑니다 — 좋은 날은 좋게, 눌리는 날은 눌리게 옵니다. 그래서 달력이 곧 지침입니다.');
    try {
      const J = gyeok(R);
