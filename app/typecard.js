...
(function (global) {
  ...
  const GOD_MEANING = {
    ...
    정인: '공인된 근거. 바른 배움과 보살핌 — 학위·자격·문서·어른의 도움처럼 제도가 인정하는 받침을 뜻한다. 받는 자리라 기댈 곳이 생기고, 소개나 추천 같은 다리가 힘을 쓴다',
  };
  const 강약뜻 = {
      ...
      if (궁본기 != null) {
        ...
        if (!궁투출) {
          ...
          문.push('배우자 방의 결론 — 방에 앉은 ' + E.STEMS[궁본기] + '(' + E.STEMS_KO[궁본기]
            ...
            : 방아쇠해.length && 둘째.달들.length ? ' 그리고 ' + E.STEMS[방아쇠] + '이 굳이 들어온다면 두 번 겹쳐야 합니다 — '
              + 방아쇠해[0] + '년 중에서도 월간에 ' + E.STEMS[방아쇠] + '이 한 번 더 뜨는 ' + 둘째.달들.join('·')
  ...
  function inyeonWhy(R) {
    ...
    // 유도·전수 검증: 열 일간 예외 0 (docs/19_법전.md 제19조).
    {
      ...
      if (접착) {
        const 합상대 = (ds + 5) % 10;
        사분면.push('먼저 구조 하나 — 당신은 ' + (남 ? '양(陽)의 기운을 타고난 남성' : '음(陰)의 기운을 타고난 여성') + '입니다. '
          + (남 ? '아내의 글자 ' : '남편의 글자 ') + E.STEMS[합상대] + '(' + E.STEMS_KO[합상대]
          + ')이 오면 당신의 일간이 먼저 손을 내밀어 맞잡습니다 — 인연을 제 손으로 붙드는 힘을 타고난 쪽입니다.');
        ...
        const 신 = (st) => E.TEN_GODS[E.tenGod(ds, st)];
        사분면.push('먼저 구조 하나 — 당신은 ' + (남 ? '음(陰)의 기운을 타고난 남성' : '양(陽)의 기운을 타고난 여성') + '입니다. '
