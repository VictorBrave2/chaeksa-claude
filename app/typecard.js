...
(function (global) {
  ...
  function 재물해점수(R, y) {
    ...
    if (g === '편재' || g === '정재') {
      ...
      sc -= 20; 샘 = true; 이유.push('나눠 갖는 손(겁재)가 온 해 — 동업·보증·큰 지출이 새기 쉬운 자리');
    } else if (g === '비견') {
    ...
    // 유도·전수 검증: 열 일간 예외 0 (docs/19_법전.md 제19조).
    {
      ...
      if (접착) {
        ...
        사분면.push('먼저 구조 하나 — ' + 기운말(R, 남, !남) + '. '
          ...
          + ')' + 조(신(손편), '이', '가') + ' 채가는 자리가 미리 예약되어 있습니다 — 인연이 오래 머물기 어렵던 구조적 이유입니다. 그래서 공주님께는 회전문을 피하는 글자와, 두 번 겹치는 때를 아는 것이 남보다 중요합니다.');
      }
      말 = 사분면.concat(말);
    }
