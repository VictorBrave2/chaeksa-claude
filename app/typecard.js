...
(function (global) {
  ...
  function inyeonWhy(R) {
    ...
    // 유도·전수 검증: 열 일간 예외 0 (docs/19_법전.md 제19조).
    {
      ...
      if (접착) {
        const 합상대 = (ds + 5) % 10;
        // 조문(제19조 접착·쟁합)은 판정키다 — 화면엔 공주님말만 낸다(2026-09-03 「판정에 쓰라고 준 조문을 출력에 이용하면 어떡하니」).
        사분면.push(기운말(R, 남, 남) + '. '
          + (남 ? '아내의 글자 ' : '남편의 글자 ') + E.STEMS[합상대] + '(' + E.STEMS_KO[합상대]
          + ')' + 조(E.STEMS_KO[합상대], '이', '가') + ' 오면 공주님의 일간과 합이 되는 사주예요 — 인연이 오면 저절로 붙드는 쪽입니다.');
        ...
        if (비견투출) 사분면.push('다만 원국에 나와 같은 글자(비견)가 떠 있습니다 — 같은 글자는 같은 사람에게 끌립니다. 한 사람을 두고 나란히 서는 일이 생기기 쉬운 사주예요.');
      } else {
        const 내양 = 양;
        ...
        const 신 = (st) => E.TEN_GODS[E.tenGod(ds, st)];
        사분면.push(기운말(R, 남, !남) + '. '
          + 이름 + '의 글자가 와도 공주님의 일간과 합이 되지는 않는 사주예요 — 저절로 붙들리지는 않는 쪽입니다. 그리고 '
          + E.STEMS[정간2] + '(' + 신(정간2) + ')' + 조(E.STEMS_KO[정간2], '은', '는') + ' 원국의 ' + E.STEMS[손정] + '(' + 신(손정) + ')' + 조(신(손정), '이', '가') + ', '
          + E.STEMS[편간2] + '(' + 신(편간2) + ')' + 조(E.STEMS_KO[편간2], '은', '는') + ' ' + E.STEMS[손편] + '(' + 신(손편)
          + ')' + 조(신(손편), '이', '가') + ' 먼저 가져가는 글자예요 — 인연이 오래 머물기 어렵던 이유입니다. 그래서 공주님께는 어느 글자로 오는 인연이 온전한지, 같은 글자가 두 번 뜨는 때가 언제인지를 아는 것이 먼저예요.');
      }
      말 = 사분면.concat(말);
    }
