...
(function (global) {
  ...
  function 연애해점수(R, y, 오행, 이름) {
    ...
    //    丁일간의 辛 편재를 丙(겁재)이 丙辛합으로 가져가고, 丁녀의 壬 정관은
    ...
    if (뺏는짝 !== undefined && E.STEM_ELEM[tf.year.stem] !== 오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + '이 당신의 ' + 이름 + ' ' + E.STEMS[뺏는짝] + '을 합으로 데려가는 해 — 삼각·이탈 조심');
    }
    ...
    if (세부.성투) {
      ...
      if (채는손 !== undefined) {
        ...
        이유.push('운이 데려온 ' + E.STEMS[tf.year.stem] + '을 원국의 ' + E.STEMS[채는손] + '이 합으로 먼저 채가는 해 — 오긴 오는데 내 것이 되기 어렵다');
      }
    }
    ...
    if (투출성.length >= 2) {
      ...
      if (걷힘 !== undefined) {
        ...
        이유.push('겹쳐 있던 ' + 이름 + ' 중 ' + E.STEMS[걷힘] + '이 걷히고 ' + E.STEMS[남는것] + '이 남는 해 — 어지럽던 인연이 맑아지는 자리');
      }
    }
  ...
  function 연애서술(d, db, 이름) {
    ...
    if (d.뺏김) 문.push('조심할 해였습니다 — 그해의 ' + 간말(d.뺏김.손) + 이가(E.STEMS_KO[d.뺏김.손]) + ' 당신의 ' + 간말(d.뺏김.성) + '을 합으로 데려가는 자리라, 곁의 인연이 흔들리거나 셋이 되기 쉬웠습니다.');
    if (d.등켜짐) 문.push('그리고 그해의 ' + 지말(d.등켜짐.지) + '는 당신의 방에서 꺼져 있던 ' + 간말(d.등켜짐.성) + '의 ' + d.등켜짐.단계 + '지 — 평소 숨죽여 있던 인연의 등이 켜진 해였습니다.');
    if (d.가로채임) 문.push('아깝게 스친 해였습니다 — 운이 인연의 글자 ' + 간말(d.가로채임.성) + 을를(E.STEMS_KO[d.가로채임.성]) + ' 데려왔는데, 당신 원국의 ' + 간말(d.가로채임.손) + 이가(E.STEMS_KO[d.가로채임.손]) + ' 합으로 먼저 채갔습니다. 좋은 사람이 지나갔는데 이상하게 이어지지 않았다면, 이 구조입니다.');
    if (d.삼합완성) 문.push('무엇보다 대운과 그해의 지지가 당신의 배우자 자리와 삼합 한 벌을 완성했습니다 — 고전이 꼽는 큰 응기의 해입니다.');
    if (d.혼잡정화) 문.push('겹쳐 있던 인연의 글자 중 ' + 간말(d.혼잡정화.걷힘) + 이가(E.STEMS_KO[d.혼잡정화.걷힘]) + ' 걷히고 ' + 간말(d.혼잡정화.남음) + '만 남는 해 — 어지럽던 것이 하나로 정리되는, 매듭짓기 좋은 자리였습니다.');
  ...
  function 재물해점수(R, y) {
    ...
    // 탈재합 — 丁일간의 辛 재성을 丙(겁재)이 丙辛합으로 데려간다. 돈이든 여자든.
    ...
    if (뺏긴재 !== undefined && E.STEM_ELEM[tf.year.stem] !== 재오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + '이 당신의 재성 ' + E.STEMS[뺏긴재] + '을 합으로 데려가는 해 — 동업·보증·빌려주기 금물');
    }
    ...
    if (g === '편재' || g === '정재') {
      ...
      if (채는손 !== undefined) {
        ...
        이유.push('운이 데려온 재성 ' + E.STEMS[tf.year.stem] + '을 원국의 ' + E.STEMS[채는손] + '이 합으로 먼저 채가는 해 — 벌이가 와도 내 몫이 줄기 쉽다');
      }
    }
  ...
  function 재물서술(d) {
    ...
    if (d.뺏김) 문.push('조심할 해였습니다 — 그해의 ' + 간말(d.뺏김.손) + 이가(E.STEMS_KO[d.뺏김.손]) + ' 당신의 재성 ' + 간말(d.뺏김.성) + '을 합으로 데려가는 자리라, 돈이든 사람이든 나눠 가자는 손이 닿기 쉬웠습니다.');
  ...
  const 다가옴_편 = '요즘 말로 불안형의 끌림입니다 — 훅 들어옵니다. 재거나 돌려 말하지 않고, 강렬하게 끌리는 대로 움직이는 쪽이라 시작이 극적입니다. 설레는 만큼 온도 조절은 당신 몫입니다.';
  ...
  function whoLovesMe(R) {
    ...
    const 위치문 = (st) => {
      const 투 = [['year', p.year.stem], ['month', p.month.stem]].concat(p.hour ? [['hour', p.hour.stem]] : [])
        .find(x => x[1] === st);
      if (투) return '이 글자는 이미 당신 원국 ' + 자리이름[투[0]] + '에 떠 있습니다 — 이 결의 사람이 벌써 삶에 들어와 있거나, 유난히 자주 만나집니다.';
      const 숨 = 지들0.find(b => (E.HIDDEN[b] || []).indexOf(st) >= 0);
      if (숨 !== undefined) return '이 글자는 당신 지지 ' + E.BRANCHES[숨] + '(' + E.BRANCHES_KO[숨] + ') 속에 숨어 있습니다 — 가까운 자리에 이미 있는데 아직 알아보지 못했기 쉽습니다.';
      return '이 글자는 당신 원국에 없습니다 — 운이 데려오는 사람입니다. 이 글자가 하늘에 뜨는 해에 만나기 쉽고, 그 해가 연애 스토리의 「열리는 해」와 같은 잣대로 짚은 자리입니다.';
    };
    ...
    const 사람 = (st, 종류) => {
      ...
      return {
        ...
        합: st === 합간,
        가로채임: 채는손 !== undefined
          ? '다만 이 글자로 오는 인연은 당신 원국의 ' + E.STEMS[채는손] + '(' + E.STEMS_KO[채는손] + ')이 합으로 먼저 채가기 쉽습니다 — 스치기 쉬운 통로라, 이 사람일수록 빨리 잡아야 합니다.'
          : null,
      };
    };
    ...
    try {
      ...
      if (궁본기 != null) {
        ...
        const 둘째달 = 궁투출 && 해들.length ? 두번달(해들[0], 방아쇠) : [];
        진사랑 = {
          회전문: 궁투출,
          ...
          기운풀이: '가장 또렷한 것은 ' + E.STEMS[궁본기] + ' 일간의 사람이지만, 일간이 아니어도 이 기운이 짙은 사주가 있습니다 — 이를테면 '
            ...
            + ')을 나란히 가져 합으로 ' + E.ELEM[e0] + '의 기운에 기운 구조도 같은 결입니다.',
          인물: STEM_PERSON[궁본기] || '',
  ...
  function 인연결론(R, now) {
      ...
      if (손 !== undefined) {
        if (그글자해.length) {
          ...
          문.push('막힌 통로 — ' + E.STEMS[st] + '(' + E.STEMS_KO[st] + '·' + 형 + ')으로 오는 인연은 원국의 '
            ...
            + (둘째.length ? ' — 다만 두 번 겹치면 뚫립니다: ' + 그글자해[0] + '년의 ' + 둘째.join('·')
              + '월은 월간에 ' + E.STEMS[st] + '이 한 번 더 떠서, 첫 글자는 소모되고 둘째 글자가 들어옵니다.' : '.'));
        }
      } else if (그글자해.length) {
      ...
      if (궁본기 != null) {
        ...
        if (!궁투출) {
          문.push('배우자 방의 결론 — 당신의 배우자 자리 ' + E.BRANCHES[db] + '에 앉은 글자는 '
            ...
            + ' 일간이 대표지만, ' + E.STEMS[k0] + '·' + E.STEMS[k0 + 5] + '을 나란히 가져 합으로 '
            + E.ELEM[e0] + '에 기운 사주도 같은 결입니다. 그 글자를 합으로 데려오는 방아쇠는 '
          ...
          const 둘째 = 방아쇠해.length ? 두번달(방아쇠해[0], 방아쇠) : [];
          문.push('배우자 방의 결론 — 방에 앉은 ' + E.STEMS[궁본기] + '(' + E.STEMS_KO[궁본기]
            + ')이 하늘에도 떠 있는 사주입니다. 그래서 ' + E.STEMS[방아쇠] + '(' + E.STEMS_KO[방아쇠]
            ...
            + (지킴 != null ? ' 이 회전문을 타지 않는 유일한 글자는 ' + E.STEMS[지킴] + '(' + E.STEMS_KO[지킴]
              + ') — 안정형의 그 통로가 방을 끝내 지킵니다.' : '')
            + (방아쇠해.length && 둘째.length ? ' 그리고 ' + E.STEMS[방아쇠] + '이 굳이 들어온다면 두 번 겹쳐야 합니다 — '
              + 방아쇠해[0] + '년 중에서도 월간에 ' + E.STEMS[방아쇠] + '이 한 번 더 뜨는 ' + 둘째.join('·')
    ...
    try {
      ...
      for (const st20 of [내양3 ? 오행 * 2 + 1 : 오행 * 2, 내양3 ? 오행 * 2 : 오행 * 2 + 1]) {
        ...
        문.push('꺼진 등의 결론 — ' + E.STEMS[st20] + '(' + E.STEMS_KO[st20] + ')은 배우자 방 ' + E.BRANCHES[db]
  ...
  // 같은 결이라도 학창 대운에서는 학창의 모습으로 말한다 (2026-08-29 교정:
  // 「직업운이 학창에 오면 학업과 아르바이트를 병행할 수 있는데 무슨 짓이냐」).
  // 빼는 게 아니라 그 나이의 말로 바꾸는 것이다.
  const 학창외면말 = {
    비겁: '또래 무리의 앞에 서던 자리 — 친구들 사이에서 내 목소리가 컸던 시절',
    식상: '만들고 표현하던 쪽 — 동아리·발표·창작, 공부보다 벌이는 일이 재밌던 시절',
    재성: '돈이 일찍 보인 결 — 아르바이트·용돈벌이를 학업과 병행했기 쉬운 시절',
    관성: '규율 안의 자리 — 반장·임원·모범생 트랙, 틀 안에서 인정받던 시절',
    인성: '책상 앞의 자리 — 공부·자격·독서로 파고들던 시절',
  };
  const 외면말 = {
  ...
  function 모습(R, now) {
    ...
    const 짓기 = du => {
      ...
      const 학창 = du.endAge <= 22;   // 대운이 대체로 학창에 걸치면 학창의 말로
      return { 구간: '만 ' + du.startAge + '~' + du.endAge + '살',
               간지: E.fmt.pillar(du), 학창,
               외면: 겉g ? 외면말[겉g] : null,
               외면문: 겉g ? (학창 ? 학창외면말[겉g] + '입니다'
                                   : 외면말[겉g] + '에 있었거나 그쪽으로 끌렸을 자리입니다') : null,
               내면: 속g ? 내면말[속g] : null };
    };
  ...
  function inyeonWhy(R) {
    ...
    // 유도·전수 검증: 열 일간 예외 0 (docs/19_법전.md 제19조).
    {
      ...
      if (접착) {
        const 합상대 = (ds + 5) % 10;
        사분면.push('먼저 구조 하나 — 당신은 ' + (남 ? '양간 남성' : '음간 여성') + '입니다. '
          + (남 ? '아내의 글자 ' : '남편의 글자 ') + E.STEMS[합상대] + '(' + E.STEMS_KO[합상대]
          + ')이 오면 당신의 일간이 직접 합으로 붙듭니다 — 인연을 붙드는 접착을 타고난 쪽입니다.');
        const 비견투출 = [p.year.stem, p.month.stem].concat(p.hour ? [p.hour.stem] : []).indexOf(ds) >= 0;
        if (비견투출) 사분면.push('다만 원국에 나와 같은 글자(비견)가 떠 있습니다 — 같은 글자는 같은 상대와 합합니다. 친구와 같은 사람 앞에 서게 되는 쟁합의 그림이니, 모임에서 시작된 인연일수록 빨리 매듭지으세요.');
      } else {
        const 내양 = 양;
        ...
        const 신 = (st) => E.TEN_GODS[E.tenGod(ds, st)];
        사분면.push('먼저 구조 하나 — 당신은 ' + (남 ? '음간 남성' : '양간 여성') + '입니다. '
          + 이름 + '과 일간이 합하지 않는, 붙드는 접착을 타고나지 않은 쪽입니다. 게다가 '
          + E.STEMS[정간2] + '(안정형)은 ' + E.STEMS[손정] + '(' + 신(손정) + ')이, '
          + E.STEMS[편간2] + '(불안형)은 ' + E.STEMS[손편] + '(' + 신(손편)
          + ')이 채가는 자리가 미리 예약되어 있습니다 — 인연이 오래 머물기 어렵던 구조적 이유입니다. 그래서 당신께는 회전문을 피하는 글자와, 두 번 겹치는 때를 아는 것이 남보다 중요합니다.');
      }
      말 = 사분면.concat(말);
    }
    ...
    // 온전한지까지 갈라 말할 수 있다. 이것이 이 진단의 값어치다.
    {
      ...
      if (막힘.length) {
        const m = 막힘[0];
        말.push('그리고 무거운 것 하나 — 원국에 ' + E.STEMS[m.손] + '(' + E.STEMS_KO[m.손] + ')이 떠 있어, '
          + E.STEMS[m.성] + '(' + E.STEMS_KO[m.성] + ')으로 오는 인연은 그 글자가 합으로 먼저 채갑니다. 좋은 사람이 와도 이상하게 내 것이 안 되던 이유가 여기 있습니다.'
          + (온전.length ? ' 대신 ' + 온전.map(st => E.STEMS[st] + '(' + E.STEMS_KO[st] + ')').join('·') + '으로 오는 인연은 온전히 당신 몫입니다 — 그 글자의 해를 잡아야 합니다.'
                          : ' 두 글자가 모두 막혀 있어, 하늘보다 땅(지지)으로 스며드는 인연이 당신의 길입니다.'));
      }
    }
    ...
    try {
      ...
      if (꺼진) {
        const jb = 장생지(꺼진.st);
        말.push(E.STEMS[꺼진.st] + '(' + E.STEMS_KO[꺼진.st] + ')은 당신의 방에서 ' + 꺼진.u
