...
(function (global) {
  ...
  /** 카드에 찍는 이름. 비면 「나」, 이미 「님」으로 끝나면 덧붙이지 않는다.
   *  옛 프로필에 이름이 '공주님'으로 저장돼 있어 카드에 **「공주님님」이 찍히고 있었다**
   *  — 호칭을 걷으면서 같이 잡았다(2026-09-10 · docs/40). 여기서 이스케이프까지 한다. */
  const 이름님 = (n) => {
    const s = String(n == null ? '' : n).trim();
    if (!s) return '나';
    const t = s.slice(-1) === '님' ? s : s + '님';
  ...
  function inyeonMonths(R, year) {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      if (E.STEM_ELEM[tf.month.stem] === 배우자오행) {
        sc += 28; 이유.push(배우자이름 + '이 달의 천간에 옵니다 — 인연의 글자입니다');
      }
      const mb = tf.month.branch;
  ...
  function myDays(R, year, month) {
    ...
    // 「좋은 날」은 **서열**이다 — 절대 문턱(≥72)은 달당 14.3일이었다(500명×12달 실측 · 신강 19.3일).
    // monthScoreFor 가 50+용신28+합22/16+강약12−눌림14 의 덧셈이라 값이 열두 개뿐이고,
    // 용신 오행 하나만 와도 78 이다. 문턱을 90 으로 올려도 11.5일 — 문턱으로는 안 갈린다.
    // 「조심할 날」(≤30)은 바닥이 36 이라 6,000달 중 **0** — 죽은 가지였다.
    // 달 안의 **최고 띠**를 좋은 날로(평균 3.0일 · 1~6일 · 97%가 100점 = 용신+합+강약 셋 다),
    // **최저 띠**(50 아래일 때만)를 조심할 날로(평균 2.7일 · 눌림이 없는 달은 0).
    // 상수를 새로 긋지 않는다(docs/13 「輕重은 비교로만」). 인연 해와 같은 태도다 — 창 안의 서열.
    const 최고 = Math.max.apply(null, rows.map(r => r.점수)), 최저 = Math.min.apply(null, rows.map(r => r.점수));
    const 좋은 = 최고 > 50 ? rows.filter(r => r.점수 === 최고) : [];
    const 조심 = 최저 < 50 ? rows.filter(r => r.점수 === 최저) : [];
  ...
  function 연애해점수(R, y, 오행, 이름) {
    ...
    if (E.STEM_ELEM[tf.year.stem] === 오행) {
      ...
      세부.성투 = true; 세부.힘 = 힘;
      이유.push(힘 > 0.8 ? 이름 + '이 뿌리까지 내리고 온 해' : 이름 + '이 천간으로 온 해');
    }
    const yb = tf.year.branch;
    ...
    if (E.isHap(ds, tf.year.stem)) {
      sc += 10; 세부.일간합 = true; 이유.push('그해 천간이 일간을 끌어당기는 합');
    }
    ...
    //    새 글자가 명령한다).
    if (세부.성투 && 인연상태(R) === '합거') {
      sc += 8; 세부.합거해방 = true; 이유.push('묶인 원국 대신 천간이 직접 데려오는 해');
    }
    ...
    if (뺏는짝 !== undefined && E.STEM_ELEM[tf.year.stem] !== 오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + 조(E.STEMS[tf.year.stem],'이','가') + ' 내 ' + 이름 + ' ' + E.STEMS[뺏는짝] + 조(E.STEMS[뺏는짝],'을','를') + ' 합으로 데려가는 해 — 삼각·이탈 조심');
    }
    ...
    if (세부.성투) {
      ...
      if (채는손 !== undefined) {
        ...
        이유.push('운이 데려온 ' + E.STEMS[tf.year.stem] + 조(E.STEMS[tf.year.stem],'을','를') + ' 원국의 ' + E.STEMS[채는손] + 조(E.STEMS[채는손],'이','가') + ' 합으로 먼저 가져가는 해 — 오긴 오는데 내 것이 되기 어렵다');
      }
    }
    // 십신 상극의 해(65·66) — 여명 식상은 관을 치고, 남명 비겁은 재를 나눈다.
    const 남2 = 이름 === '재성';
    const 세십신 = E.TEN_GODS[E.tenGod(ds, tf.year.stem)];
    // 14조 식상견관·비겁탈재 배경 — 2026-09-04 법전에서 뺌(사장님 「법 있는 것으로만」). 코드는 원본 커밋에 있다.
    // 12조 삼합 완성 — 2026-09-04 법전에서 뺌(사장님 「법 있는 것으로만」). 코드는 원본 커밋에 있다.
    // 13조 혼잡 정화 — 2026-09-04 법전에서 뺌(사장님 「법 있는 것으로만」). 코드는 원본 커밋에 있다.
  ...
  function 연애서술(d, db, 이름) {
    const 문 = [];
    if (d.성투) {
      문.push('그해(' + d.간지 + ') 천간에 내 인연 글자 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 떠올랐습니다'
        + (d.힘 > 0.8 ? ' — 뿌리까지 내려 힘이 실린 채로, 스치는 호감이 아니라 이어질 힘이 있는 만남으로 옵니다.'
                       : ' — 인연이 눈앞에 모습을 드러내는 해입니다.'));
    }
    if (d.관계 === '육합') 문.push('그리고 그해의 ' + 지말(d.세지) + 이가(E.BRANCHES_KO[d.세지]) + ' 내 배우자 자리 ' + 지말(db) + 을를(E.BRANCHES_KO[db]) + ' 끌어안는 합이었습니다 — 마음이 기울고 곁을 내주게 되는 해입니다.');
    else if (d.관계 === '삼합') 문.push('그해의 ' + 지말(d.세지) + '는 내 배우자 자리 ' + 지말(db) + '와 삼합으로 맞물립니다 — 같이 다니고 같이 겪으며 가까워지는 해입니다.');
    // 관계에는 육합·삼합만 들어간다(981·983줄) — 충 가지는 제23조 뒤로 죽은 코드였다.
    if (d.암장) 문.push('그해는 천간이 아니라 지지로도 왔습니다 — ' + 지말(d.세지) + ' 속에 인연의 글자가 숨어 들어, 소문 없이 가까워지는 종류의 해입니다.');
    if (d.도화) 문.push('게다가 그해의 ' + 지말(d.세지) + '는 내 도화(桃花) 자리 — 가만히 있어도 눈에 띄고, 끌리는 해입니다.');
    if (d.일간합) 문.push('무엇보다 그해의 천간 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 내 일간을 곧장 끌어당기는 합이라, 머리보다 마음이 먼저 움직입니다.');
    if (d.조후) 문.push('그리고 그 글자는 내 사주가 계절적으로 목말라 하던 약이기도 해서, 삶 전체가 풀리며 인연도 함께 열립니다.');
    if (d.합거해방) 문.push('원국의 인연 글자는 합으로 묶여 있지만, 이런 해는 천간이 새 글자를 직접 데려옵니다 — 묶임과 무관하게 움직이는 해입니다.');
    if (d.뺏김) 문.push('조심할 해였습니다 — 그해의 ' + 간말(d.뺏김.손) + 이가(E.STEMS_KO[d.뺏김.손]) + ' 내 ' + 간말(d.뺏김.성) + '을 합으로 데려가는 해라, 곁의 인연이 흔들리거나 셋이 되기 쉬웠습니다.');
    if (d.등켜짐) 문.push('그리고 그해의 ' + 지말(d.등켜짐.지) + '는 내 방에서 꺼져 있던 ' + 간말(d.등켜짐.성) + '의 ' + d.등켜짐.단계 + '지 — 평소 숨죽여 있던 인연의 등이 켜진 해였습니다.');
    if (d.가로채임) 문.push('아깝게 스친 해였습니다 — 운이 인연의 글자 ' + 간말(d.가로채임.성) + 을를(E.STEMS_KO[d.가로채임.성]) + ' 데려왔는데, 내 원국의 ' + 간말(d.가로채임.손) + 이가(E.STEMS_KO[d.가로채임.손]) + ' 합으로 먼저 채갔습니다. 좋은 사람이 지나갔는데 이상하게 이어지지 않았다면, 이 구조입니다.');
    // 12·13조 문장 — 2026-09-04 법전에서 뺌
  ...
  function 재물해점수(R, y) {
    ...
    if (g === '편재' || g === '정재') {
      ...
      else 이유.push(힘 > 0.8 ? '재성이 뿌리까지 내리고 온 해 — 크게 들어옵니다' : '재성이 천간으로 온 해');
      sc += add;
    } else if (g === '식신' || g === '상관') {
    ...
    if (뺏긴재 !== undefined && E.STEM_ELEM[tf.year.stem] !== 재오행) {
      ...
      이유.push('세운 ' + E.STEMS[tf.year.stem] + 조(E.STEMS[tf.year.stem],'이','가') + ' 내 재성 ' + E.STEMS[뺏긴재] + 조(E.STEMS[뺏긴재],'을','를') + ' 합으로 데려가는 해 — 동업·보증·빌려주기 금물');
    }
    ...
    if (g === '편재' || g === '정재') {
      ...
      if (채는손 !== undefined) {
        ...
        이유.push('운이 데려온 재성 ' + E.STEMS[tf.year.stem] + 조(E.STEMS[tf.year.stem],'을','를') + ' 원국의 ' + E.STEMS[채는손] + 조(E.STEMS[채는손],'이','가') + ' 합으로 먼저 가져가는 해 — 벌이가 와도 내 몫이 줄기 쉽다');
      }
    }
  ...
  function 재물서술(d) {
    const 문 = [];
    if (d.십신 === '편재' || d.십신 === '정재') {
      문.push('그해(' + d.간지 + ') 천간에 내 돈 글자 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 떠올랐습니다'
        + (d.힘 > 0.8 ? ' — 뿌리까지 내려, 스쳐 가는 돈이 아니라 쥐어지는 돈이 오는 해입니다.'
                       : ' — 벌이의 기회가 눈에 보이게 나타나는 해입니다.'));
      if (d.신약) 문.push('다만 힘이 밖에서 채워지는 사주라, 들어온 만큼 몸이 바쁘고 버거움도 같이 왔기 쉽습니다.');
    } else if (d.십신 === '식신' || d.십신 === '상관') {
      문.push('그해는 ' + 간말(d.세간) + ' — 내가 만들어내는 기운의 해였습니다. 일이 돈이 되는 길목이 놓여, 이때 벌인 판이 뒤의 밑천이 됩니다.');
    }
    if (d.뿌리) 문.push('게다가 그해의 ' + 지말(d.세지) + '로 돈의 뿌리가 지지까지 들어와, 바닥이 받쳐 준 해였습니다.');
    if (d.암장) 문.push('그해는 지지로도 왔습니다 — ' + 지말(d.세지) + ' 속에 돈의 글자가 숨어 들어, 겉으론 조용한데 통장은 움직이는 종류의 해입니다.');
    if (d.일간합) 문.push('그해의 천간 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 내 일간과 합 — 돈이 굴러오는 게 아니라 내 손이 먼저 뻗는 해입니다.');
    if (d.조후) 문.push('그 글자는 내 사주가 계절적으로 목말라 하던 약이기도 해서, 일이 전반적으로 풀리며 벌이도 함께 열립니다.');
    if (d.뺏김) 문.push('조심할 해였습니다 — 그해의 ' + 간말(d.뺏김.손) + 이가(E.STEMS_KO[d.뺏김.손]) + ' 내 재성 ' + 간말(d.뺏김.성) + '을 합으로 데려가는 자리라, 돈이든 사람이든 나눠 가자는 손이 닿기 쉬웠습니다.');
  ...
  function 재물월점수(R, y, m) {
    ...
    let sc = 30; const 이유 = [];
    const g = E.TEN_GODS[E.tenGod(ds, tf.month.stem)];
    if (g === '편재' || g === '정재') { sc += 26; 이유.push('재성이 달의 천간에 옵니다 — 돈이 도는 달'); }
  ...
  function wealthWhy(R) {
    ...
    if (투.length) {
      ...
      if (합거[k]) {
        말 = ['돈의 글자(재성)가 원국 천간에 있는데, 다른 글자와 합으로 묶여 있습니다.',
              '내 돈인데 내 마음대로 안 되는 형국입니다 — 벌어도 어딘가에 걸려 있기 쉽습니다.',
              '이 묶임을 흔드는 운이 오는 해에 풀립니다. 그래서 「언제」가 액수보다 먼저입니다.'];
      } else if (E.stemPower(st, 자리) === 0) {
        말 = ['돈의 글자(재성)가 천간에 있는데, 지지에 뿌리가 없습니다.',
              '뿌리 없는 재물은 스쳐 지나갑니다 — 들어온 것 같은데 남지 않았던 이유가 여기 있습니다.',
              '이 글자에 뿌리가 들어오는 해 — 그때가 쥐는 해입니다. 그 해를 알아야 합니다.'];
      } else {
        말 = ['돈의 기운이 겉에 드러나 있고 뿌리까지 있어요. 버는 힘은 타고난 사주예요.',
      ...
      말 = ['돈의 글자(재성)이 천간에는 없고 지지 속에 숨어 있습니다.',
            '이 글자가 천간으로 오는 해에 벌립니다. 그 해를 놓치지 않는 것이 이 사주의 전부입니다.'];
    } else {
    ...
    if (겁재투) 말.push('한 가지 더 — 나눠 갖는 겁재가 원국 천간에 있습니다. 벌리는 해에는 그 겁재도 같이 움직입니다. 새는 해를 아는 것이 버는 해를 아는 것만큼 중요합니다.');
    return { 말 };
  }
  ...
  const GOD_MEANING = {
    ...
    // 貴賤은 폐기했다(docs/27 하나) — 「명예·권위·인정받는」은 벼슬의 말이다. 官은 영역이다:
    // 규율을 받는 대신 자리를 받는 것. 편관은 실질로 쥐는 자리, 정관은 제도가 인정하는 자리.
    편관: '나를 강하게 누르는 대신 자리를 주는 힘. 압박·단련·실권을 뜻한다. 감당하면 실질로 쥐는 자리가 되고, 못 감당하면 압박과 소모만 남는다. 여성에게는 강한 남성의 글자이기도 하다',
    정관: '나를 반듯하게 누르는 대신 자리를 주는 힘. 질서·직함·자격을 뜻한다. 제도가 인정하는 자리·공식적인 관계에 좋고, 여성에게는 남편의 글자다',
  ...
  const 강약뜻 = {
    신약: '일간을 돕는 기운이 적다는 뜻. 모자란 게 아니라 밖에서 채워지는 구조다 — 좋은 사람과 좋은 때를 만나면 몇 배로 가고, 혼자 소모전을 하면 빨리 지친다. 그래서 이 사주는 「때」를 아는 것이 힘의 절반이다',
  ...
  /** 배우자성 글자의 십신 이름(정관·편관·정재·편재). 「안정형·불안형」은 2026-09-04 폐기 —
   *  원전 십신에 우리가 붙인 이름이었다. 한 곳에서만 정한다. */
  function 형말(R, st) {
    try {
      const E2 = global.ChaeksaEngine;
      return E2.TEN_GODS[E2.tenGod(R.pillars.day.stem, st)];
    } catch (e) { return ''; }
  }
  ...
  function 인연결론(R, now) {
    ...
    [오행 * 2, 오행 * 2 + 1].forEach(st => {
      ...
      // 십신 이름 그대로(정관·편관·정재·편재). 「안정형·불안형」은 2026-09-04 폐기.
      const 형 = E.TEN_GODS[E.tenGod(ds, st)];
      if (손 !== undefined) {
        if (그글자해.length) {
          const 둘째 = 두번충족(R, 그글자해[0], st);
          // 조문 용어(통로·채가다·뚫리다)는 판정키다 — 화면엔 공주님말만(2026-09-03).
          문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + '·' + 형 + ')' + 조(E.STEMS_KO[st], '으로', '로') + ' 오는 인연은 원국의 '
            + E.STEMS[손] + '(' + E.STEMS_KO[손] + ')' + 이가(E.STEMS_KO[손]) + ' 합으로 먼저 가져갑니다. 그 글자가 오는 '
            + 그글자해.join('·') + '년은 스치기 쉬운 해예요'
            + (둘째.겹 ? ' — 다만 ' + 그글자해[0] + '년은 대운 천간에 이미 ' + E.STEMS[st]
              + 조(E.STEMS[st], '이', '가') + ' 있어, 세운의 ' + E.STEMS[st] + '까지 두 번이 됩니다. 첫 글자는 원국이 가져가고 둘째 글자가 들어오는, 해 전체가 열리는 해예요.'
            : 둘째.달들.length ? ' — 다만 같은 글자가 두 번 오면 들어옵니다. ' + 그글자해[0] + '년의 ' + 둘째.달들.join('·')
              + '월은 월간에 ' + E.STEMS[st] + 조(E.STEMS[st],'이','가') + ' 한 번 더 와서, 첫 글자는 원국이 가져가고 둘째 글자가 들어와요.' : '.'));
        }
      } else if (그글자해.length) {
        문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + '·' + 형 + ')' + 조(E.STEMS_KO[st], '으로', '로') + ' 오는 인연은 아무도 가로채지 못합니다. 그 글자가 천간에 오는 '
          + 그글자해.join('·') + '년이 내 해예요.'
          + 올해면(그글자해, now));
      }
    });
    ...
    if (해방해.length) 문.push('묶임이 풀리는 해 — 원국의 ' + 이름 + '은 합으로 묶여 있지만, '
      + 해방해.join('·') + '년에는 천간이 새 글자를 직접 데려옵니다. 묶임과 무관하게 움직이는 해입니다.');
    // 12·13조 결론 — 2026-09-04 법전에서 뺌(사장님 「법 있는 것으로만」). 코드는 원본 커밋에 있다.
    ...
    try {
      ...
      if (궁본기 != null) {
        ...
        if (!궁투출) {
          문.push('배우자 방의 결론 — 내 배우자 자리 ' + E.BRANCHES[db] + '에 앉은 글자는 '
            ...
            + E.ELEM[e0] + '에 기운 사주도 같습니다. 그 글자를 합으로 불러오는 글자는 '
            + E.STEMS[방아쇠] + (맞물림 ? ' — 내 ' + 이름 + '이기도 해서 자리와 글자가 맞물린 사주예요' : '')
          ...
          문.push('배우자 방의 결론 — 방에 앉은 ' + E.STEMS[궁본기] + '(' + E.STEMS_KO[궁본기]
            + ')' + 조(E.STEMS_KO[궁본기], '이', '가') + ' 천간에도 있는 사주입니다. 그래서 ' + E.STEMS[방아쇠] + '(' + E.STEMS_KO[방아쇠]
            + ')의 사람이 방에 들어오려 하면 천간의 ' + E.STEMS[궁본기]
            + 조(E.STEMS[궁본기], '과', '와') + ' 합이 되어 들어왔다 나가는 자리가 됩니다 — 인연이 자리를 못 잡던 이유예요.'
            + (지킴 != null ? ' 이 자리를 타지 않는 유일한 글자는 ' + E.STEMS[지킴] + '(' + E.STEMS_KO[지킴]
              + '·' + 형말(R, 지킴) + ') — 그 글자가 방을 끝내 지킵니다.' : '')
            + (방아쇠해.length && 둘째.겹 ? ' 그리고 ' + 방아쇠해[0] + '년은 대운 천간에 이미 ' + E.STEMS[방아쇠]
              + 조(E.STEMS[방아쇠], '이', '가') + ' 있어 세운까지 두 번이 됩니다 — 해 전체가 문을 여는 해예요.'
            : 방아쇠해.length && 둘째.달들.length ? ' 그리고 ' + E.STEMS[방아쇠] + 조(E.STEMS[방아쇠],'이','가') + ' 굳이 들어온다면 같은 글자가 두 번 와야 합니다. '
              + 방아쇠해[0] + '년 중에서도 월간에 ' + E.STEMS[방아쇠] + 조(E.STEMS[방아쇠],'이','가') + ' 한 번 더 오는 달이 ' + 둘째.달들.join('·')
              + '월이에요. 첫 글자는 원국이 합으로 가져가고, 둘째 글자가 문을 엽니다.' : ''));
        }
      }
    } catch (e) {}
    ...
    try {
      ...
      for (const st20 of [내양3 ? 오행 * 2 + 1 : 오행 * 2, 내양3 ? 오행 * 2 : 오행 * 2 + 1]) {
        ...
        try {
          ...
          if (du2 && du2.branch === jb) 대운글 = ' 지금 대운의 지지가 바로 ' + E.BRANCHES[jb] + ' — 등이 켜져 있는 10년 위에 계십니다.';
        } catch (e) {}
        문.push('꺼진 등의 결론 — ' + E.STEMS[st20] + '(' + E.STEMS_KO[st20] + ')' + 조(E.STEMS_KO[st20], '은', '는') + ' 배우자 방 ' + E.BRANCHES[db]
          ...
          + (켜지는해.length ? ' 세운으로는 ' + 켜지는해.join('·') + '년에 지지로 옵니다.' : ''));
        break;
      }
    } catch (e) {}
  ...
  function 재물결론(R, now) {
    ...
    [재오행 * 2, 재오행 * 2 + 1].forEach(st => {
      ...
      if (손 !== undefined) {
        if (그글자해.length) 문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + ')으로 오는 돈은 원국의 '
          + E.STEMS[손] + '(' + E.STEMS_KO[손] + ')' + 이가(E.STEMS_KO[손]) + ' 합으로 먼저 가져갑니다. ' + 그글자해.join('·')
          + '년의 벌이는 정산이 야박하기 쉬우니 계약서를 두껍게 쓰세요.');
      } else if (그글자해.length) {
        문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + ')으로 오는 돈은 아무도 가로채지 못합니다 — 온전히 내 몫이에요. '
  ...
  function reading(R, kind, now) {
    ...
    const 원장 = {
      ...
      머리: love ? [
        ...
        '기준을 정합니다 — 투출·통근·합거·도화·조후, 지난 연표를 짚은 기준 그대로',
      ] : [
        '원국을 폅니다 — ' + 사주줄,
        '돈의 글자: 재성 · 새는 손: 겁재' + (v.강약 === '신약' ? ' · 신약이라 재성 가산을 줄여 잽니다' : ''),
        '기준을 정합니다 — 재성 투출·식상·겁재·조후, 지난 연표를 짚은 기준 그대로',
      ],
      달줄: 달들.map(m => ({
    ...
    for (let y = 태어난해 + 17; y < 올해; y++) {
      ...
      if (ys === 궁본기) {
        ...
        사건들.push({ 해: y, 나이: a, 점수: 합됨 ? 9 : 8, 축: '관계',
          ...
          근거: '배우자 방에만 있던 글자 ' + 간말(ys) + 이가(E.STEMS_KO[ys]) + ' 이 해 겉으로 드러났습니다' + (합됨 ? ' — 그리고 내 원국의 글자가 바로 합으로 잡았습니다' : '') + '.' });
      }
      ...
      if (겁재떠 && (ys + 5) % 10 === 겁재간 && E.STEM_ELEM[ys] === 재오행)
        사건들.push({ 해: y, 나이: a, 점수: 9, 축: '재물',
          ...
          근거: '돈의 글자 ' + 간말(ys) + 이가(E.STEMS_KO[ys]) + ' 온 해인데, 원국 천간에 있는 ' + 간말(겁재간) + '(겁재)가 먼저 합으로 데려갔습니다. 여기만은 방향이 글자에 적혀 있습니다 — 내 몫이 남에게 가는 해였습니다.' });
      ...
      if (((ys + 5) % 10 === ds || (ds + 5) % 10 === ys) && E.TEN_GODS[E.tenGod(ds, ys)] === '정관')
        사건들.push({ 해: y, 나이: a, 점수: 8, 축: '자리',
          ...
          근거: '소속의 글자 ' + E.STEMS[ys] + '(정관)이 이 해 내 일간과 직접 합했습니다. 방향은 단정하지 않습니다.' });
      ...
      if (E.STEM_ELEM[ys] === 재오행 && E.BRANCH_ELEM[yb] === 재오행)
        사건들.push({ 해: y, 나이: a, 점수: 6, 축: '재물',
          ...
          근거: '돈의 글자 ' + 간말(ys) + 이가(E.STEMS_KO[ys]) + ' 뿌리(' + E.BRANCHES[yb] + ')까지 갖추고 온 해입니다. 늘었는지 나갔는지는 본인이 아십니다.' });
    }
  ...
  function inyeonWhy(R) {
    ...
    if (투.length) {
      ...
      if (합거[k]) {
        상태 = '합거';
        말 = [글자 + 조(이름,'이','가') + ' 원국 천간에 있는데, 다른 글자와 합으로 묶여 있습니다.',
              '묶인 글자는 제 노릇을 못 합니다 — 인연이 곁을 지나가도 내 것으로 매듭이 잘 안 지어지는 형국입니다.',
              '이건 흠이 아니라 시계입니다. 이 묶임을 흔드는 운이 오는 해에 몰아서 움직입니다. 그래서 「언제」가 유난히 무겁습니다.'];
      } else if (E.stemPower(st, 자리) === 0) {
        상태 = '무근';
        말 = [글자 + 조(이름,'은','는') + ' 천간에 있는데, 지지에 뿌리가 없습니다.',
              '뿌리 없는 글자는 시작은 만들어도 오래 잇지 못합니다 — 만남은 있는데 매듭이 안 지어졌던 이유가 여기 있습니다.',
              '이 글자에 뿌리가 되어 주는 운이 들어오는 해 — 그때가 진짜입니다. 그래서 해와 달을 골라야 합니다.'];
      } else {
        상태 = '유근';
        말 = ['인연의 기운이 겉에 드러나 있고 뿌리까지 있어요. 사람이 눈에 잘 띄는 사주예요.',
              '이런 사주는 사람이 오느냐가 문제가 아니에요. 여럿 중에 어느 사람을 잡아서 언제 매듭짓느냐가 문제예요.',
              '인연의 기운이 겹치거나 짝 자리가 움직이는 때에 갈림길이 와요. 그 때를 미리 알아 두는 게 이 사주에는 요령이에요.'];
      }
    } else if (암) {
      상태 = '암장';
      말 = [글자 + 조(이름,'이','가') + ' 천간에는 없고 지지 속에 숨어 있습니다.',
            '숨은 글자는 밖으로 드러나질 않아서, 좋은 사람이 곁에 있어도 잘 안 보이고 잘 안 잡힙니다.',
            '이 글자가 천간으로 오는 해 — 그때 비로소 보이고 잡힙니다. 그래서 「언제」를 아는 것이 곧 「누구」를 아는 것입니다.'];
    } else {
      상태 = '무';
    ...
    // 19조 일간합 사분면 — 2026-09-04 법전에서 뺌(사장님 「법 있는 것으로만」). 코드는 원본 커밋에 있다.
    ...
    // 온전한지까지 갈라 말할 수 있다. 이것이 이 진단의 값어치다.
    {
      ...
      if (막힘.length) {
        const m = 막힘[0];
        말.push('그리고 무거운 것 하나 — 원국 천간에 ' + E.STEMS[m.손] + '(' + E.STEMS_KO[m.손] + ')' + 이가(E.STEMS_KO[m.손]) + ' 있어, '
          + E.STEMS[m.성] + '(' + E.STEMS_KO[m.성] + ')으로 오는 인연은 그 글자가 합으로 먼저 가져갑니다. 좋은 사람이 와도 이상하게 내 것이 안 되던 이유가 여기 있습니다.'
          + (온전.length ? ' 대신 ' + 온전.map(st => E.STEMS[st] + '(' + E.STEMS_KO[st] + ')').join('·') + '으로 오는 인연은 온전히 내 몫이에요 — 그 글자의 해를 잡아야 합니다.'
                          : ' 두 글자가 모두 막혀 있어, 천간보다 지지로 스며드는 인연이 내 길입니다.'));
      }
    }
    ...
    try {
      ...
      let 꺼진 = null;
      [정간3, 편간3].forEach(st => {
        const u = E.unseong(st, db);
        if ((u === '절' || u === '묘') && !꺼진) 꺼진 = { st, u };
        줄.push(E.STEMS[st] + '(' + E.TEN_GODS[E.tenGod(ds, st)] + ')·' + u);
      });
      말.push('배우자 방(' + E.BRANCHES[db] + ')에서 인연의 글자가 얼마나 살아 있나 — ' + 줄.join(' / ') + '.');
      if (꺼진) {
        const jb = 장생지(꺼진.st);
        말.push(E.STEMS[꺼진.st] + '(' + E.STEMS_KO[꺼진.st] + ')은 내 방에서 ' + 꺼진.u
          + ' — 꺼져 있는 등입니다. 쓸 만한 사람이 오래 머물지 않는다고 느껴 오셨다면, 맞게 느끼신 겁니다. 다만 결함이 아니라 스위치가 따로 있는 등입니다'
          + (jb != null ? ' — ' + E.BRANCHES[jb] + '(' + E.BRANCHES_KO[jb] + ')가 지지로 오는 해와 대운에 이 등이 켜집니다.' : '.'));
      }
    } catch (e) {}
    return { 상태, 이름, 말 };
  }
  ...
  function monthWhy(R) {
    const a = R.analysis || E.strengthOf(R.pillars);
    const 말 = [];
    if (a.strength === '신약') 말.push('내 사주는 쓸 수 있는 힘이 정해져 있는 쪽입니다. 아무 날에나 벌이면 새고, 받쳐주는 날에 몰아치면 남습니다 — 그래서 날의 서열이 값진 사주입니다.');
    else if (a.strength === '신강') 말.push('내 사주는 힘이 안에 고이는 쪽입니다. 내보낼 곳이 있는 날에 움직여야 풀리고, 고이는 날에 웅크리면 답답해집니다 — 그 날들을 갈라 드립니다.');
    else 말.push('내 사주는 치우침이 적어 그날 오는 기운을 그대로 탑니다 — 좋은 날은 좋게, 눌리는 날은 눌리게 옵니다. 그래서 달력이 곧 지침입니다.');
    try {
      const J = gyeok(R);
  ...
  const ELEM_PERSON = {
    목: ['자라는 기운', '먼저 벌이고 밀고 나가는 쪽입니다. 계획을 세우고 사람을 모읍니다',
         ['甲', '乙'], '밀어주는 사람'],
    화: ['밝히는 기운', '드러내고 표현하는 쪽입니다. 곁에 있으면 분위기가 환해집니다',
         ['丙', '丁'], '환하게 해주는 사람'],
    토: ['받쳐주는 기운', '중재하고 품는 쪽입니다. 흔들릴 때 자리를 지켜 줍니다',
         ['戊', '己'], '받쳐주는 사람'],
    금: ['정리하는 기운', '끊고 맺는 쪽입니다. 어지러운 것을 갈라 세워 줍니다',
         ['庚', '辛'], '정리해주는 사람'],
    수: ['흐르게 하는 기운', '궁리하고 이어주는 쪽입니다. 막힌 데를 돌아가게 합니다',
         ['壬', '癸'], '이어주는 사람'],
  };
  const GOD_LEAN = {
        ...
        if (가짐) {
          ...
          온도 = { 종류: '거스름', 글자: gA.기신,
            말:님(nameB) + '의 기운은 ' + 님(nameA) + '께 거슬리는 쪽(' + gA.기신 + ')입니다 — '
              + '나쁜 사람이라는 뜻이 아니라, 오래 붙어 있으면 이쪽이 더워지거나 추워집니다' };
        }
      }
    } catch (e) {}
  ...
  function 영역축(힘, 주력, R) {
    ...
    재기('비겁', 겁,   // 官剋我 — 자리와 나의 크기
      '맡은 것보다 내가 더 큽니다. 자리가 나를 다 못 담는 쪽이에요.',
      '맡은 것이 나보다 무겁습니다. 감당하는 데 힘이 실립니다.');
    재기('식상', 식,   // 食傷剋官 — 내놓는 것과 자리의 마찰
      '하고 싶은 말과 재주가 자리보다 세요. 그게 앞설수록 자리가 흔들리는 상황이 자주 생겨요.',
      '하고 싶은 것이 자리를 흔들지는 않아요. 자리 안에서 가는 편이에요.',
      '하고 싶은 것이 겉으로는 안 나와 있어요. 자리를 흔들 게 밖에 없는 사주예요.');
    ...
    재기('인성', 인,   // 官生印 — 자리가 나에게 돌려주는가
      '맡은 것이 내 안에 쌓입니다. 경력이 되고 명분이 됩니다.',
  ...
  function 자리내력(힘) {
    ...
    return { 갈: '맡겨짐', 말: '맡겨지는 자리 쪽입니다 — 받치는 것도 미는 것도 겉에 없이 자리만 홀로 있어요. 오면 그대로 맡게 되는 쪽입니다.' };
  }
  ...
  // (글자 잣대) 자리가 없는 사람들 — 친구·경쟁자·스승·시어머니 — 는 여전히 글자로 본다.
  // 아버지·어머니·배우자·자식은 아래 궁위 잣대(관계지도)가 맡는다. 2026-09-02 사장님 조문.
  function 관계지도_글자(R, fromYear) {
    ...
    const 사람들 = 육친표[여명 ? 'F' : 'M'].map(([이름, 신들, 뜻]) => {
      ...
      let 말;
      if (위치 === '겉') 말 = 글이가 + ' ' + 궁말[궁] + '에 드러나 있습니다.';
      else if (위치 === '묶임') 말 = 글이가 + ' ' + 궁말[궁] + '에 있는데 합으로 묶여 있습니다. 있는데 힘을 못 쓰는 쪽이에요 — 묶임을 푸는 운에 살아납니다.';
      else if (위치 === '속') 말 = 글은는 + ' 겉으로 안 나오고 ' + 궁말[궁] + ' 속에 있습니다. 평소엔 조용하다가 그 글자가 천간에 오는 해에 드러나는 쪽이에요.';
      else 말 = 글이가 + ' 겉으로도 속으로도 안 나와 있습니다. 없다는 뜻이 아니라 내 안에 자리를 안 잡은 것이에요 — 운이 데려올 때만 만납니다.';
      if (해 != null) 말 += ' ' + (해 === y0 ? '올해' : 해 + '년') + ' 그 글자가 천간으로 옵니다.';
      // 한 줄 — 조립기(성아)가 쓴다. 위치만, 궁 이름은 짧게.
      const 궁짧 = 궁 ? { year: '연주', month: '월주', day: '일지', hour: '시주' }[궁] : '';
      const 짧게 = 위치 === '겉' ? 이름.split('·')[0] + '의 글자는 ' + 궁짧 + '에 드러나 있습니다.'
        : 위치 === '묶임' ? 이름.split('·')[0] + '의 글자는 ' + 궁짧 + '에 있는데 묶여 있습니다.'
        : 위치 === '속' ? 이름.split('·')[0] + '의 글자는 ' + 궁짧 + ' 속에 있습니다.'
        : 이름.split('·')[0] + '의 글자는 겉으로 안 나와 있습니다.';
      return { 이름, 신들, 뜻, 위치, 궁, 힘, 무게: null, 해, 말, 짧게 };
    });
    // ── 어머니와 나 — 제25조 「인성은 근거, 어머니는 존재의 근거」 ──
    // 위치에 강약을 얹으면 한 문장이 더 나온다. 새 계산 없음 — 억부의 저울(DRAIN)과 같은 값이다:
    //   신강 + 어머니 겉   받기만 하면 답답한 사람(형준 「받기만 할 때」)에게 근거가 겉에 서 있다
    //   신약 + 어머니 겉   받쳐 줄 근거가 겉에 있다 — 기댈 곳이 밖에 드러나 있다
    //   신약 + 어머니 속/없음   받쳐 줄 근거가 겉에 없다 — 밖에서 찾게 되는 쪽
    //   신강 + 어머니 속/없음   근거를 안 받아도 서는 쪽 — 그래서 어머니와 거리가 편하다
    try {
      const 엄 = 사람들.filter(s => s.이름 === '어머니')[0];
      const 강 = (R.analysis || E.strengthOf(p)).strength;
      if (엄) {
        const 겉임 = 엄.위치 === '겉';
        엄.깊이 = 강 === '신강'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 드러나 있는데 나는 받기만 하면 답답해지는 쪽이라, 어머니 곁에서 내보낼 곳이 있어야 편합니다.'
                  : '어머니는 근거의 글자입니다 — 나는 근거를 안 받아도 서는 쪽이라, 어머니와 거리가 있는 편이 서로 편합니다.')
          : 강 === '신약'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 나는 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 드러나 있습니다. 기댈 곳이 밖에 드러나 있는 사주예요.'
                  : '어머니는 근거의 글자입니다 — 나는 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 없습니다. 받침을 밖에서 찾게 되는 사주예요.')
          : (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 드러나 있고 나는 치우침이 없는 쪽이라, 있는 만큼 받고 있는 만큼 돌려주는 사이예요.'
                  : '어머니는 근거의 글자입니다 — 겉에는 없고 나는 치우침이 없는 쪽이라, 필요할 때만 서로 찾는 사이예요.');
        엄.말 += ' ' + 엄.깊이;
      }
    } catch (e) {}
    // 무게는 서열이다 — 「나보다 가볍다」는 85%에게 나갔다(일간은 합, 남의 천간은 최대로 재니 늘 일간이 무겁다).
    // 안 갈리는 말은 안 한다. 겉에 선 사람이 둘 이상이면 그중 **가장 무거운 글자** 하나만 짚는다.
    const 겉들 = 사람들.filter(s => s.위치 === '겉');
    if (겉들.length >= 2) {
      const 톱 = 겉들.slice().sort((a, b) => b.힘 - a.힘)[0];
      if (톱.힘 > 0) { 톱.무게 = '가장 무겁다'; 톱.말 = 톱.말.replace('드러나 있습니다.', '드러나 있습니다. 곁에 있는 사람 중 가장 무거운 글자예요 — 그 기운이 나에게 제일 많이 얹혀 있는 쪽입니다.'); }
    }
    return { 여명, 사람들,
      잣대: '육친은 통설을 씁니다 — ' + (여명 ? '정재=시어머니 · 편관=남편 외 남자는' : '편재=애인 · 관성=자녀는') + ' 유파에 따라 갈립니다. 무게는 그 글자의 뿌리 힘과 일간의 힘을 견준 것이고, 좋고 나쁨이 아닙니다.' };
  }

  // ── 관계지도 — 궁이 사람이고, 그 자리에 앉은 글자가 그 사람의 모양이다 (docs/30 · 사장님 조문 2026-09-02) ──
  //
  //   월간  아버지     「정관 같은 아버지」 「정인 같은 아버지」 …
  //   월지  어머니     「정관 같은 어머니」 …             (지지 궁은 본기 십신 — 배우자궁과 같은 관례)
  //   일지  배우자     (인연·궁합이 이미 깊이 본다 — 여기서는 모양만)
  //   시간  정신세계   사람이 아니라 속 — 그 글자의 결로 짜인 정신세계
  //   시지  자식       「시지 같은 자식」
  //   년주  아직 조문을 안 받았다 — 조상·초년으로만 두고 사람을 안 붙인다
  //
  // 글자로 사람을 찾던 어제의 표(편재=아버지)는 거꾸로였다 — 「아버지의 글자가 없습니다」가 나왔다.
  // 궁은 늘 채워져 있으니 **「없다」가 없다.** 있고 없고가 아니라 어떤 아버지인가를 말한다.
  // 상태는 자리의 상태다: 천간 궁은 합거(묶임), 지지 궁은 원국 안의 합·충(제23조는 운의 충이지 원국의 충이 아니다).
  const 모양표 = {   // 「〇〇 같은 사람」 — 십신 열 개. 성격 서술이지 좋고 나쁨이 아니다.
    정관: '반듯하고 규율로 서는', 편관: '세고 누르는, 실권을 쥔',
    정인: '근거가 되어 주고 받쳐 주는', 편인: '제 방식이 있고 옆문을 아는',
    정재: '차곡차곡 지키고 실속을 챙기는', 편재: '움직이고 판을 벌이는',
    식신: '여유롭고 먹이고 즐기게 하는', 상관: '말이 앞서고 튀는, 재주 있는',
    비견: '나와 같은 기운으로 나란히 서는', 겁재: '겨루고 나눠 갖는',
  };
  const 정신표 = {   // 시간 = 정신세계 — 속이 무슨 결로 짜였나
    정관: '질서와 규율로 짜인', 편관: '긴장과 각오로 짜인', 정인: '배움과 근거로 짜인', 편인: '궁리와 직관으로 짜인',
    정재: '실속과 관리로 짜인', 편재: '움직임과 기회로 짜인', 식신: '여유와 즐거움으로 짜인', 상관: '표현과 파격으로 짜인',
    비견: '제 페이스로 짜인', 겁재: '겨룸과 승부로 짜인',
  };
  function 관계지도(R, fromYear) {
    ...
    // 천간은 드러난 것이라 첫인상, 지지는 바닥이라 자란 집. 조상·가문·국가는 공주님이 확인할 길이 없어 안 쓴다.
    // 년지는 부모가 아니라 「집의 공기」다 — 부모는 월주가 말한다.
    const 궁들 = [
      { 이름: '첫인상', 궁: 'year', 천간: true, 사람아님: true, 첫인상: true },
      { 이름: '자란 집', 궁: 'year', 천간: false, 사람아님: true, 집: true },
      { 이름: '아버지', 궁: 'month', 천간: true },
      { 이름: '어머니', 궁: 'month', 천간: false },
      { 이름: '배우자', 궁: 'day', 천간: false },
      { 이름: '정신세계', 궁: 'hour', 천간: true, 사람아님: true },
      { 이름: '자식', 궁: 'hour', 천간: false },
    ];
    const 궁사람들 = 궁들.map(q => {
      const pl = p[q.궁];
      if (!pl) return { 이름: q.이름, 궁: q.궁, 천간: q.천간, 십신: null, 말: q.이름 + '는 시주에 있는데 태어난 시각을 몰라 비워 둡니다.' };
      ...
      if (q.첫인상) 말 = '사람들은 나를 먼저 ' + 독음 + '(' + 글자 + ') — ' + g + '의 글자로 봅니다. 속과 다를 수 있습니다.';
      ...
      if (해 != null && !q.사람아님) 말 += ' ' + (해 === y0 ? '올해' : 해 + '년') + ' 그 글자가 천간으로 옵니다.';
      return { 이름: q.이름, 궁: q.궁, 궁이름, 천간: q.천간, 글자, 십신: g, 모양: g ? g + '의 글자' : null, 상태, 해, 말 };
    });
    // 어머니 — 제25조. 월지의 글자가 인성이면 근거의 어머니다. 강약을 얹어 한 문장 더.
    try {
      const 엄 = 궁사람들.filter(s => s.이름 === '어머니')[0];
      if (엄 && (엄.십신 === '정인' || 엄.십신 === '편인')) {
        const 강 = (R.analysis || E.strengthOf(p)).strength;
        엄.깊이 = 강 === '신강' ? '어머니가 근거의 자리에 앉았는데 나는 받기만 하면 답답해지는 쪽이라, 어머니 곁에서 내보낼 곳이 있어야 편합니다.'
               : 강 === '신약' ? '어머니가 근거의 자리에 앉았고 나는 받쳐 줄 것이 필요한 쪽이라, 기댈 곳이 어머니 자리에 있는 사주예요.'
               : '어머니가 근거의 자리에 앉았고 나는 치우침이 없는 쪽이라, 있는 만큼 받고 있는 만큼 돌려주는 사이예요.';
        엄.말 += ' ' + 엄.깊이;
      }
    } catch (e) {}
    // 자리가 없는 사람들은 글자로 — 형제·친구 · 경쟁자 · 스승 · 시어머니(女·통설)
    let 글자사람들 = [];
    try {
      const 남는 = 여명 ? ['형제·친구', '경쟁자·동업자', '스승·비스듬한 어른', '시어머니'] : ['형제·친구', '경쟁자·동업자', '스승·비스듬한 어른'];
      글자사람들 = 관계지도_글자(R, y0).사람들.filter(s => 남는.indexOf(s.이름) >= 0);
    } catch (e) {}
    return { 여명, 궁사람들, 글자사람들,
      잣대: '궁이 사람이고 그 자리에 앉은 글자가 그 사람의 모양입니다 — 월간 아버지 · 월지 어머니 · 일지 배우자 · 시간 정신세계 · 시지 자식. 지지 궁은 본기 글자로 읽습니다. 자리가 없는 사람(친구·경쟁자·스승' + (여명 ? '·시어머니' : '') + ')은 글자로 봅니다' + (여명 ? ' — 정재=시어머니는 통설이라 유파에 따라 갈립니다' : '') + '. 좋고 나쁨이 아니라 모양과 자리입니다.' };
  }
  ...
  function 자리모양(R) {
    ...
    const 말 = [];
    말.push(종류 === '혼잡'
      ? '정관과 편관이 같이 드러나 있습니다. 제도가 준 자리와 실질로 쥔 자리가 둘이라, 지켜야 할 것이 둘인 쪽이에요.'
      : 종류 === '정관'
