...
(function (global) {
  ...
  function 연애서술(d, db, 이름) {
    const 문 = [];
    if (d.성투) {
      문.push('그해(' + d.간지 + ') 하늘에 내 인연 글자 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 떠올랐습니다'
    ...
    if (d.일간합) 문.push('무엇보다 그해의 하늘 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 내 일간을 곧장 끌어당기는 합이라, 머리보다 마음이 먼저 움직입니다.');
  ...
  function 재물서술(d) {
    const 문 = [];
    if (d.십신 === '편재' || d.십신 === '정재') {
      문.push('그해(' + d.간지 + ') 하늘에 내 돈 글자 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 떠올랐습니다'
    ...
    if (d.일간합) 문.push('그해의 하늘 ' + 간말(d.세간) + 이가(E.STEMS_KO[d.세간]) + ' 내 일간과 합 — 돈이 굴러오는 게 아니라 내 손이 먼저 뻗는 해입니다.');
    ...
    try {
      ...
      if (du) {
        ...
        d.대운 = { 간지: gz(du), 십신: g,
          ...
          뜻: '대운은 10년 단위의 큰 바탕이다. 지금 이 사람은 ' + g + '의 10년 위에 있다 — ' + (GOD_MEANING[g] || '') };
      }
    } catch (e) {}
    try {
  ...
  function 인연결론(R, now) {
    ...
    [오행 * 2, 오행 * 2 + 1].forEach(st => {
      ...
      if (손 !== undefined) {
        ...
        문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + '·' + 형 + ')' + 조(E.STEMS_KO[st], '으로', '로') + ' 오는 인연은 아무도 가로채지 못합니다. 그 글자가 하늘에 오는 '
          + 그글자해.join('·') + '년이 내 해예요.'
          + 올해면(그글자해, now));
      }
    });
  ...
  function inyeonWhy(R) {
    ...
    if (투.length) {
      ...
      말 = [글자 + 조(이름,'이','가') + ' 하늘에는 없고 지지 속에 숨어 있습니다.',
            ...
            '이 글자가 하늘에 뜨는 해 — 그때 비로소 보이고 잡힙니다. 그래서 「언제」를 아는 것이 곧 「누구」를 아는 것입니다.'];
    } else {
      상태 = '무';
      ...
      if (막힘.length) {
        ...
        말.push('그리고 무거운 것 하나 — 원국에 ' + E.STEMS[m.손] + '(' + E.STEMS_KO[m.손] + ')' + 이가(E.STEMS_KO[m.손]) + ' 떠 있어, '
          ...
          + (온전.length ? ' 대신 ' + 온전.map(st => E.STEMS[st] + '(' + E.STEMS_KO[st] + ')').join('·') + '으로 오는 인연은 온전히 내 몫이에요 — 그 글자의 해를 잡아야 합니다.'
                          : ' 두 글자가 모두 막혀 있어, 하늘보다 땅(지지)으로 스며드는 인연이 내 길입니다.'));
      }
    }
