  ...
  function 관계지도_글자(R, fromYear) {
    ...
    try {
      ...
      if (엄) {
        const 겉임 = 엄.위치 === '겉';
        엄.깊이 = 강 === '신강'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 서 있는데 나는 받기만 하면 답답해지는 쪽이라, 어머니 곁에서 내보낼 곳이 있어야 편합니다.'
                  : '어머니는 근거의 글자입니다 — 나는 근거를 안 받아도 서는 쪽이라, 어머니와 거리가 있는 편이 서로 편합니다.')
          : 강 === '신약'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 나는 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 서 있습니다. 기댈 곳이 밖에 드러나 있는 사주예요.'
                  : '어머니는 근거의 글자입니다 — 나는 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 없습니다. 받침을 밖에서 찾게 되는 사주예요.')
          : (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 서 있고 나는 치우침이 없는 쪽이라, 있는 만큼 받고 있는 만큼 돌려주는 사이예요.'
    ...
    if (겉들.length >= 2) {
      ...
      if (톱.힘 > 0) { 톱.무게 = '가장 무겁다'; 톱.말 = 톱.말.replace('드러나 서 있습니다.', '드러나 서 있습니다. 곁에 선 사람 중 가장 무거운 글자예요 — 그 기운이 나에게 제일 많이 얹혀 있는 쪽입니다.'); }
    }
    return { 여명, 사람들,
