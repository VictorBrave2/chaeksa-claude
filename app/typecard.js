...
(function (global) {
  ...
  function whoLovesMe(R) {
    ...
    const 사람 = (st, 종류) => {
      ...
      return {
        ...
        합: st === 합간,
        가로채임: 채는손 !== undefined
          ? '다만 이 글자로 오는 인연은 공주님 원국의 ' + E.STEMS[채는손] + '(' + E.STEMS_KO[채는손] + ')' + 이가(E.STEMS_KO[채는손]) + ' 합으로 먼저 가져가기 쉽습니다 — 스치기 쉬운 글자라, 이 사람일수록 빨리 잡아야 합니다.'
          : null,
      };
    };
      ...
      if (궁본기 != null) {
        ...
        if (!궁투출) {
          문.push('배우자 방의 결론 — 공주님의 배우자 자리 ' + E.BRANCHES[db] + '에 앉은 글자는 '
            ...
            + E.STEMS[방아쇠] + (맞물림 ? ' — 공주님의 ' + 이름 + '이기도 해서 자리와 글자가 맞물린 사주예요' : '')
  ...
  function 재물결론(R, now) {
    ...
    [재오행 * 2, 재오행 * 2 + 1].forEach(st => {
      ...
      if (손 !== undefined) {
        ...
        문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + ')으로 오는 돈은 아무도 가로채지 못합니다 — 온전히 공주님 몫이에요. '
    ...
    if (투.length) {
      ...
      if (합거[k]) {
        ...
        말 = [글자 + 조(이름,'이','가') + ' 원국 하늘에 떠 있는데, 다른 글자와 합으로 묶여 있습니다.',
              ...
              '이건 흠이 아니라 시계입니다. 이 묶임을 흔드는 운이 오는 해에 몰아서 움직입니다. 그래서 공주님께는 「언제」가 유난히 무겁습니다.'];
      } else if (E.stemPower(st, 자리) === 0) {
        상태 = '무근';
    ...
    // 온전한지까지 갈라 말할 수 있다. 이것이 이 진단의 값어치다.
    {
      ...
      if (막힘.length) {
        ...
        말.push('그리고 무거운 것 하나 — 원국에 ' + E.STEMS[m.손] + '(' + E.STEMS_KO[m.손] + ')' + 이가(E.STEMS_KO[m.손]) + ' 떠 있어, '
          ...
          + (온전.length ? ' 대신 ' + 온전.map(st => E.STEMS[st] + '(' + E.STEMS_KO[st] + ')').join('·') + '으로 오는 인연은 온전히 공주님 몫이에요 — 그 글자의 해를 잡아야 합니다.'
  ...
  function monthWhy(R) {
    const a = R.analysis || E.strengthOf(R.pillars);
    const 말 = [];
    if (a.strength === '신약') 말.push('공주님 사주는 쓸 수 있는 힘이 정해져 있는 쪽입니다. 아무 날에나 벌이면 새고, 받쳐주는 날에 몰아치면 남습니다 — 그래서 날의 서열이 값진 사주입니다.');
  ...
  function 관계지도_글자(R, fromYear) {
    ...
    try {
      ...
      if (엄) {
        const 겉임 = 엄.위치 === '겉';
        엄.깊이 = 강 === '신강'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 서 있는데 공주님은 받기만 하면 답답해지는 쪽이라, 어머니 곁에서 내보낼 곳이 있어야 편합니다.'
                  : '어머니는 근거의 글자입니다 — 공주님은 근거를 안 받아도 서는 쪽이라, 어머니와 거리가 있는 편이 서로 편합니다.')
          : 강 === '신약'
          ? (겉임 ? '어머니는 근거의 글자입니다 — 공주님은 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 서 있습니다. 기댈 곳이 밖에 드러나 있는 사주예요.'
                  : '어머니는 근거의 글자입니다 — 공주님은 받쳐 줄 것이 필요한 쪽인데 그 근거가 겉에 없습니다. 받침을 밖에서 찾게 되는 사주예요.')
          : (겉임 ? '어머니는 근거의 글자입니다 — 그 근거가 겉에 서 있고 공주님은 치우침이 없는 쪽이라, 있는 만큼 받고 있는 만큼 돌려주는 사이예요.'
                  : '어머니는 근거의 글자입니다 — 겉에는 없고 공주님은 치우침이 없는 쪽이라, 필요할 때만 서로 찾는 사이예요.');
        엄.말 += ' ' + 엄.깊이;
      }
    } catch (e) {}
  ...
  function 관계지도(R, fromYear) {
    ...
    try {
      ...
      if (엄 && (엄.십신 === '정인' || 엄.십신 === '편인')) {
        const 강 = (R.analysis || E.strengthOf(p)).strength;
        엄.깊이 = 강 === '신강' ? '어머니가 근거의 자리에 앉았는데 공주님은 받기만 하면 답답해지는 쪽이라, 어머니 곁에서 내보낼 곳이 있어야 편합니다.'
               : 강 === '신약' ? '어머니가 근거의 자리에 앉았고 공주님은 받쳐 줄 것이 필요한 쪽이라, 기댈 곳이 어머니 자리에 있는 사주예요.'
               : '어머니가 근거의 자리에 앉았고 공주님은 치우침이 없는 쪽이라, 있는 만큼 받고 있는 만큼 돌려주는 사이예요.';
        엄.말 += ' ' + 엄.깊이;
      }
    } catch (e) {}
