...
(function (global) {
  ...
  function 연애해점수(R, y, 오행, 이름) {
    ...
    if (E.STEM_ELEM[tf.year.stem] === 오행) {
      ...
      세부.성투 = true; 세부.힘 = 힘;
      이유.push(힘 > 0.8 ? 이름 + '이 뿌리까지 내리고 온 해' : 이름 + '이 천간에 뜬 해');
    }
    const yb = tf.year.branch;
  ...
  function 재물해점수(R, y) {
    ...
    if (g === '편재' || g === '정재') {
      ...
      else 이유.push(힘 > 0.8 ? '재성이 뿌리까지 내리고 온 해 — 크게 들어옵니다' : '재성이 천간에 뜬 해');
      sc += add;
    } else if (g === '식신' || g === '상관') {
  ...
  function wealthWhy(R) {
    ...
    if (투.length) {
      ...
      if (합거[k]) {
        ...
        말 = ['돈의 글자(재성)이 천간에 떠 있는데, 뿌리가 없습니다.',
      ...
      말 = ['돈의 글자(재성)이 천간에는 없고 지지 속에 숨어 있습니다.',
            '이 글자가 천간에 뜨는 해에 벌립니다. 그 해를 놓치지 않는 것이 이 사주의 전부입니다.'];
    } else {
    ...
    if (겁재투) 말.push('한 가지 더 — 나눠 갖는 손(겁재)가 원국 천간에 떠 있습니다. 벌리는 해에는 그 손도 같이 옵니다. 새는 해를 아는 것이 버는 해를 아는 것만큼 중요합니다.');
    return { 말 };
  }
        ...
        if (그글자해.length) {
          ...
          문.push(E.STEMS[st] + '(' + E.STEMS_KO[st] + '·' + 형 + ')' + 조(E.STEMS_KO[st], '으로', '로') + ' 오는 인연은 원국의 '
            ...
            + (둘째.겹 ? ' — 다만 ' + 그글자해[0] + '년은 대운 천간에 이미 ' + E.STEMS[st]
              + 조(E.STEMS[st], '이', '가') + ' 떠 있어, 세운의 ' + E.STEMS[st] + '까지 두 번이 됩니다. 첫 글자는 원국이 가져가고 둘째 글자가 들어오는, 해 전체가 열리는 해예요.'
            : 둘째.달들.length ? ' — 다만 같은 글자가 두 번 뜨면 들어옵니다. ' + 그글자해[0] + '년의 ' + 둘째.달들.join('·')
              + '월은 월간에 ' + E.STEMS[st] + 조(E.STEMS[st],'이','가') + ' 한 번 더 떠서, 첫 글자는 원국이 가져가고 둘째 글자가 들어와요.' : '.'));
        }
      } else if (그글자해.length) {
      ...
      if (궁본기 != null) {
        ...
        if (!궁투출) {
          ...
          문.push('배우자 방의 결론 — 방에 앉은 ' + E.STEMS[궁본기] + '(' + E.STEMS_KO[궁본기]
            + ')' + 조(E.STEMS_KO[궁본기], '이', '가') + ' 천간에도 떠 있는 사주입니다. 그래서 ' + E.STEMS[방아쇠] + '(' + E.STEMS_KO[방아쇠]
            ...
            + (방아쇠해.length && 둘째.겹 ? ' 그리고 ' + 방아쇠해[0] + '년은 대운 천간에 이미 ' + E.STEMS[방아쇠]
              + 조(E.STEMS[방아쇠], '이', '가') + ' 떠 있어 세운까지 두 번이 됩니다 — 해 전체가 문을 여는 해예요.'
            : 방아쇠해.length && 둘째.달들.length ? ' 그리고 ' + E.STEMS[방아쇠] + 조(E.STEMS[방아쇠],'이','가') + ' 굳이 들어온다면 같은 글자가 두 번 떠야 합니다. '
  ...
  function inyeonWhy(R) {
    ...
    if (투.length) {
      ...
      말 = [글자 + 조(이름,'이','가') + ' 천간에는 없고 지지 속에 숨어 있습니다.',
            ...
            '이 글자가 천간에 뜨는 해 — 그때 비로소 보이고 잡힙니다. 그래서 「언제」를 아는 것이 곧 「누구」를 아는 것입니다.'];
    } else {
      상태 = '무';
