...
(function (global) {
  ...
  function deltaT(year) {
    const y = year;
    let t;
    if (y < 1800) { t = (y - 1820) / 100; return -20 + 32 * t * t; }
  ...
  /** 태양 시황경(도). jdUT: 세계시 율리우스일 */
  function apparentSunLongitude(jdUT) {
    const year = 2000 + (jdUT - 2451545.0) / 365.25;
    const jde = jdUT + deltaT(year) / 86400;
    const t = (jde - 2451545.0) / 365250; // 율리우스 천년
    const L = (sum(L0,t) + sum(L1,t)*t + sum(L2,t)*t**2 + sum(L3,t)*t**3 + sum(L4,t)*t**4 + sum(L5,t)*t**5) / 1e8; // rad
    const R = (sum(R0,t) + sum(R1,t)*t + sum(R2,t)*t**2 + sum(R3,t)*t**3 + sum(R4,t)*t**4) / 1e8; // AU
    let lon = L * 180 / Math.PI + 180;          // 지심 황경
    // FK5 보정
    const T = t * 10;
    const lamP = lon - 1.397 * T - 0.00031 * T * T;
    lon += -0.09033 / 3600 + 0; // 경도 보정(위도 항은 무시)
    // 장동(Δψ) 간이식
    const omega = (125.04452 - 1934.136261 * T) * D2R;
    const Ls = (280.4665 + 36000.7698 * T) * D2R, Lm = (218.3165 + 481267.8813 * T) * D2R;
    const dpsi = (-17.20 * Math.sin(omega) - 1.32 * Math.sin(2 * Ls) - 0.23 * Math.sin(2 * Lm) + 0.21 * Math.sin(2 * omega)) / 3600;
    // 광행차
    const ab = -20.4898 / 3600 / R;
    lon = lon + dpsi + ab;
    void lamP;
