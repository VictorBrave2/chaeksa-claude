...
(function (global) {
  ...
  const 바라는사람 = { 비견: '친구처럼 편하고 대등한 사람', 겁재: '같이 신나 주는 사람', 식신: '같이 있으면 마음이 편하고 여유로운 사람', 상관: '말이 잘 통하는 사람', 편재: '여기저기 같이 놀러 다녀 주는 사람', 정재: '꼼꼼하게 생활을 챙겨 주는 사람', 편관: '이끌어 주고 지켜 주는 사람', 정관: '반듯하고 약속을 지키는 사람', 편인: '말 안 해도 알아채 주는 사람', 정인: '따뜻하게 받아 주는 사람' };
  const 좋아하는법 = { 식신: '챙겨 주는 것', 상관: '하고 싶은 말을 바로 하고 앞장서는 것' };
  /* 72조 ⑨(09-25 사장님 「육합을 제외시키고 진행해」 · 「변질된 글자를 추종해야」): 일지는 삼합의 기운을 따라간다.
   *  丑寅辰巳未申戌亥는 삼합 국의 한 끝이라, 그 국의 왕지(子午卯酉)가 운으로 오면 국의 기운으로 끌려가고 지장간 속 그 기운의 글자가 나선다
   *  (辰 · 申 + 子 → 水 癸 · 壬 / 丑 · 巳 + 酉 → 金 辛 · 庚 / 未 · 亥 + 卯 → 木 乙 · 甲 / 戌 · 寅 + 午 → 火 丁 · 丙). 운이 지나면 돌아온다 — 만남 · 이별 · 재회.
   *  子午卯酉는 국의 가운데라 끌려갈 곳이 없다 — 이상형이 일정하다(변질 없음). 육합으로는 변질을 재지 않는다. */
  const 삼합끌림 = { 4: [0, 4], 8: [0, 4], 1: [9, 3], 5: [9, 3], 7: [3, 0], 11: [3, 0], 10: [6, 1], 2: [6, 1] };   // 일지 → [끌어가는 왕지, 국의 오행]
  function 일지변질(R) {
    const ds = R.pillars.day.stem, br = R.pillars.day.branch, 속 = (E.HIDDEN[br] || []).map(h => typeof h === 'number' ? h : h[0]);
    const 끌 = 삼합끌림[br]; if (!끌) return null;
    const 본 = 속[0], 후 = 속.find(h => E.STEM_ELEM[h] === 끌[1]);
    const 신 = (st) => E.TEN_GODS[E.tenGod(ds, st)];
    return 후 == null ? null : { 전: 신(본), 후: 신(후), 전글자: E.STEMS[본], 후글자: E.STEMS[후], 짝: E.BRANCHES[끌[0]] };
  }
    ...
    ['year', 'month', 'hour'].forEach(k => { if (!p[k]) return; const g = p[k].stem, 십 = 신(g); if (십 !== '식신' && 십 !== '상관') return;
      층들.forEach(u => { if ((u.stem - g + 10) % 10 !== 5) return;
        ...
        언.push({ 키: E.STEMS[나섬] + 후, 글자: E.STEMS[나섬], 십신: 후, 드러남: true, 새: true, 층: u.name, 변질: true,
          말: 언제 + '에는 좋아하는 방식이 바뀌어요. 평소엔 ' + 좋아하는법[십] + '으로 마음을 보이는데, 이때는 ' + 좋아하는법[후] + '으로 보여요.' }); }); });
    return { t, 바람, 언 };
  }
