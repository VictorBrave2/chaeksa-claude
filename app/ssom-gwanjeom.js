...
(function (global) {
  ...
  const 밀어내는것 = { 비견: '친구와 내 일', 겁재: '친구와 내 일', 식신: '하고 싶은 일과 놀 거리', 상관: '하고 싶은 일과 놀 거리', 정재: '현실 일과 돈', 편재: '현실 일과 돈', 정관: '일의 책임', 편관: '일의 책임', 정인: '생각할 거리와 집안 일', 편인: '생각할 거리와 집안 일' };
  function 때풀이(R, y, m) {
    ...
    층들.forEach(u => {
      ...
      if (u.name === '대운') {
        if (관 === '반합') 바람.push('대운 ' + 지 + '가 일지와 합을 맺어, 이 열 해는 짝에게 바라는 마음이 평소보다 커져 있어요.');
        else if (관 === '육합') 바람.push('대운 ' + 지 + '가 일지를 합으로 붙들어, 이 열 해는 바라는 마음이 쉽게 흔들리지 않아요.');
        else if (관 === '충') 바람.push('대운 ' + 지 + '가 일지를 쳐서, 이 열 해는 바라는 마음이 자주 흔들려요.');
      } else if (u.name === '세운') {
        if ((록왕[ds] || []).indexOf(u.branch) >= 0) 바람.push(y + '년은 ' + 지 + ' 해라 스스로 힘이 서는 해예요. 기대고 싶은 마음이 한결 누그러져요.');
        if (관 === '반합' || 관 === '육합') 바람.push(y + '년은 바라는 마음이 한층 또렷한 해예요.');
        else if (관 === '충') 바람.push(y + '년은 바라는 마음이 흔들리는 해예요.');
      } else if (u.name === '월운') {
        if (관 === '충') { const h = (E.HIDDEN[u.branch] || [])[0], st = typeof h === 'number' ? h : h[0], 신 = E.TEN_GODS[E.tenGod(ds, st)];
          바람.push(m + '월은 ' + 지 + '가 일지를 쳐서, 바라는 마음이 ' + (밀어내는것[신] || '다른 일') + '에 밀려 잠깐 뒤로 물러나는 달이에요. 마음이 바뀐 게 아니라 밀려난 거예요.' + (붙듦 ? ' 대운 · 세운의 합이 붙들고 있어 크게 흔들리진 않아요.' : '')); }
        else if (관 === '반합' || 관 === '육합') 바람.push(m + '월은 바라는 마음이 더 또렷해지는 달이에요.');
      }
    });
    ...
    Object.keys(묶음).forEach(k => { const x = 묶음[k], 제목 = 조각[x.키] ? 조각[x.키].제목 : (x.십신 === '식신' ? '챙기는 모습' : '방법을 내놓고 표현하는 모습');
      ...
      x.말 = x.곳.join(' · ') + (x.드러남 ? '의 천간으로 ' : ' 속에 ') + E.STEMS_KO[E.STEMS.indexOf(x.글자)] + '(' + x.글자 + ') ' + x.십신 + '이 와서 「' + 제목 + '」 — 이 모습이 ' + (새 ? '새로 ' : '한층 ') + (x.드러남 ? '겉으로 드러나요.' : '속으로 더해져요.');
      언.push(x); });
    return { t, 바람, 언 };
  }
  ...
  function 때견줌(R, 그때, 이제) {
    ...
    const 조각 = global.ChaeksaSsomJuneun || {}, 이름 = (v) => (조각[v.키] ? 조각[v.키].제목 : v.키) + (v.드러남 ? '(겉으로)' : '(속으로)');
    빠짐(그때).forEach(v => 말.push(v.새
      ? '그때는 ' + v.층 + '이 더해 준 모습이 있었어요 — 「' + 이름(v) + '」. 지금은 빠졌으니, 그때 같은 모습은 지금 기대하기 어려워요.'
      : '그때는 ' + v.층 + '이 보태 「' + 이름(v) + '」가 더 뚜렷했는데, 지금은 타고난 만큼으로 돌아왔어요.'));
    새로.forEach(v => 말.push(v.새 ? '지금은 ' + v.층 + '이 새 모습을 더해 줘요 — 「' + 이름(v) + '」. 그때는 없던 모습이에요.' : '지금은 ' + v.층 + '이 보태 「' + 이름(v) + '」가 그때보다 뚜렷해요.'));
    const 흔들 = (x) => x.바람.some(s => /밀려|흔들/.test(s));
    if (!흔들(그때) && 흔들(이제)) 말.push('바라는 마음은 그때는 흔들리지 않았는데, 지금은 흔들리거나 밀려나 있어요.');
    else if (흔들(그때) && !흔들(이제)) 말.push('바라는 마음은 그때는 흔들렸는데, 지금은 원래대로 돌아왔어요.');
    if (!말.length) 말.push('그때와 지금, 바라는 마음도 언행도 크게 달라지지 않았어요.');
    return 말;
  }
  ...
  global.ChaeksaSsom = { 언행, 때상태, 때풀이, 때견줌, 지관계, 닿음키, 구성말, 식상, 일지: 일지지, 일주, 일지십신, 방향, 짝, 질문, 반응틀, 반응물음, 기록: 일지, 적기 };
})(window);
