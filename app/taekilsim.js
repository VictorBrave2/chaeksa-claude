 ...
 *   ③ 관점이 내준 말을 그린다
 * 점수 · 순위 · 등수를 내지 않는다(60조). 줄은 「세 눈 모두 걸리는 것 없음」이라는 조건으로만 거른다.
 *
...
(function (global) {
  ...
  function render(box, opts) {
    ...
    box.innerHTML = '<section class="card tk" data-plain="1">'
      ...
      + '<label class="tk-filter"><input type="checkbox" id="tkClean"> 세 눈 모두 걸리는 것 없는 자리만 보기</label>'
    ...
    function 그리기() {
      ...
      q('tkTop').textContent = W.출산택일.머리 + ' ' + r.곳 + ' 기준 시계 시각이에요(이날은 시계가 해보다 ' + Math.abs(r.밀림) + '분 ' + (r.밀림 >= 0 ? '빨라요' : '늦어요') + '). '
        + (깨끗 ? '이날은 세 눈 모두 걸리는 것 없는 자리가 ' + 깨끗 + '곳 있어요.' : '이날은 세 눈이 다 걸리는 것 없다고 본 자리가 없어요. 앞뒤 날도 보세요.');
