...
(function (global) {
  ...
  M.판차 = (R, 기준해, 볼해) => {
    ...
    const 키 = M.무리들.map(g => { const d = b.x[g] - a.x[g]; return d > 0 ? '+' : d < 0 ? '-' : '0'; }).join('');
    // 합계 보존 — 네 글자를 세므로 두 상태의 합이 같으면 델타의 합이 0이고,
    // 그러면 키에 `+` 와 `-` 가 함께 있거나 둘 다 없다(00000).
    // 한쪽만 있으면 세다가 흘린 것이다(대운 없는 나이·십신 분류 실패). 조용히 넘기지 않는다.
    const 합 = (o) => M.무리들.reduce((s, g) => s + o.x[g], 0);
    const 든것 = 합(a), 볼것 = 합(b);
    const 어긋남 = (든것 !== 볼것) || ((키.indexOf('+') >= 0) !== (키.indexOf('-') >= 0));
    return { 키, 기준: a, 볼: b, 합: { 기준: 든것, 볼: 볼것 }, 어긋남,
             델타: M.무리들.reduce((o, g) => (o[g] = b.x[g] - a.x[g], o), {}) };
  };
