    ...
    Object.keys(W.뼈대).forEach(k => {
      ...
      (w || []).forEach((회, i) => {
        ...
        회.forEach(([누, 말]) => {
          ...
          if (누 === '단서' || 누 === '근거') return;
          if (금지.test(말)) 걸림.push(Object.assign({ 종류: '금지 낱말', 말 }, 곳));
          if (한자.test(말)) 걸림.push(Object.assign({ 종류: '단서 밖 한자', 말 }, 곳));
          if (사주말.test(말)) 걸림.push(Object.assign({ 종류: '해설 · 대사의 사주 말', 말 }, 곳));
          if (조사틀림.test(말)) 걸림.push(Object.assign({ 종류: '조사', 말 }, 곳));
          if (누 === '인연' && 코칭.test(말)) 걸림.push(Object.assign({ 종류: '코칭 말투', 말 }, 곳));
        ...
        if (해설 > 6) 걸림.push({ 종류: '한 회 책사 해설 너무 김(' + 해설 + '줄)', 단계: k, 회: i + 1, 말: '' });
      });
    });
    Object.keys(방향말).forEach(말 => { if (방향말[말] > 1) 걸림.push({ 종류: '한 방향 같은 해설 되풀이(' + 방향말[말] + '번)', 단계: '', 회: 0, 말 }); });
    if (버릇.근데 > 3) 걸림.push({ 종류: '버릇 말 「근데」 ' + 버릇.근데 + '번', 단계: '', 회: 0, 말: '' });
    return 걸림;
  }
  W.검사 = 검사;
})(window);
