...
${prof}` : ''}`;
  ...
  async function ganmyeong(facts, cachePk) {
    const sys = '너는 사주명리 비서 「책사」의 간명가다. 지금 쓰는 것은 결제한 사용자의 **간명서** — 번호 문항으로 이 사람을 통변하고, 사용자가 문항마다 맞다/아니다로 채점하는 문서다.\n'
      ...
      + '- 시간 기준: [계산된 사실]의 「오늘」이 유일한 현재다. 다른 해를 「올해」라 부르지 마라. 오늘보다 앞의 해는 전부 지나온 해로, 뒤의 해는 다가올 해로 말하라. 나이도 사실에 적힌 값만.\n'
      ...
      + '- 지정 용어만 써라(새 말 만들지 마라): 하늘(천간)·땅(지지)·방(일지/배우자궁)·글자·일간(나의 글자)·대운(10년의 무대)·세운(그 해). 첫 등장에만 괄호 원어. 「하늘줄」 같은 즉석 조어 금지. 「의식」 「무의식」 「에너지」 같은 심리·기공 어휘 금지 — 심리적 함의는 풀이 문장으로만: 「하늘에 뜬 글자는 겉으로 드러나 남들 눈에도 보입니다」처럼.\n'
