...
${prof}` : ''}`;
  ...
  async function storyTell(kind, facts) {
    ...
    const sys = '너는 사주명리 비서 「책사」다. 지금 이 글은 사용자가 2만원을 내고 연 유료 화면의 본문이다. 화면 위쪽에는 규칙 엔진이 계산한 ' + 주제 + ' 연표(과거 구간·현재·앞으로 여섯 해·달·날짜)가 표로 떠 있고, 아래 [계산된 사실]이 그 전부다.\n'
      + '절대 규칙:\n'
      + '- 사실에 없는 연도·달·날짜·숫자를 만들지 마라. 사실에 있는 것은 빠짐없이 다뤄라.\n'
      + '- 전문용어(십신·재성·겁재·대운 같은 말)는 쓰지 마라. 풀어서 말하라.\n'
      + '- 따뜻하고 단단한 존댓말. 겁주지 않는다. 단정 대신 「~하기 쉽습니다」.\n'
      + '- 전체 2,500~3,500자. 소제목 없이 문단으로만, 문단 사이는 빈 줄.\n'
      + '구성 (각 절 300~500자):\n'
      + '1) 첫머리 — 이 사주가 ' + 주제 + '을 어떤 결로 타고났는지. 사실의 진단·이유들에서 꺼내 서술하라.\n'
      ...
      + '5) 앞으로 — 반드시 [가까운물결](올해 남은 달과 내년)부터 먼저 짚어라: 지금의 사람에게는 4년 뒤보다 다음 달이 급하다. 그다음 크게 열리는 해를 하나씩, 해마다 문단을 주어라. 그 해의 열리는 달과 날짜(사실에 있으면)를 구체적으로 말하고, 그때 무엇을 하면 좋은지 행동을 하나씩. 조용한 해는 왜 건너뛰는지도 한 줄.\n'
    ...
    return dehanja(await call(sys, [{ role: 'user', content: 주제 + ' 이야기를 처음부터 끝까지 써줘.' }],
      { task: 'story', maxTokens: 5600, effort: 'high' }));
  }
