...
${prof}` : ''}`;
  ...
  async function storyTell(kind, facts) {
    ...
    const sys = '너는 「책사단」의 기록자다. 지금 이 글은 공주님이 값을 치르고 여신 ' + 주제 + ' 화면의 본문이다. 화면 위쪽에는 규칙 엔진이 계산한 연표(과거 구간·현재·다가오는 열두 달·날·시진)가 표로 떠 있고, 아래 [계산된 사실]이 그 전부다.\n'
      ...
      + '- 전체 900~1,400자. **짧게 써라.** 사실·날짜·표는 화면 위에 이미 다 있다. 그것을 다시 늘어놓으면 이 글은 값이 없다.\n'
      ...
      + '화자 배분(각 문단 맨 앞에 〔이름〕):\n'
      + '1) 〔협기〕 결론부터. 열두 달 가운데 어느 달인지, 그중에서도 어느 날 어느 시각인지. 첫 문단이 답이어야 한다.\n'
      + '2) 〔' + (kind === 'wealth' ? '재물' : '인연') + '〕 왜 그 달인가. [결론]에서 하나만 골라 풀어라 — 전부 옮기지 마라.\n'
      + '3) 〔협기〕 그 달에 무엇을 하시면 되는지. 고른 날에 무엇을 하고 무엇을 피하는지.\n'
    ...
    return await call(sys, [{ role: 'user', content: 주제 + ' 이야기를 처음부터 끝까지 써줘.' }],
      { task: 'story', maxTokens: 3000, effort: 'medium' });
  }
