...
(function (global) {
  ...
  function 그리기(box, input) {
    ...
    if (SM) {
      안전(4, '십이운성', '내가 ' + (시없음 ? '세' : '네') + ' 자리에서 각각 얼마나 힘이 있나', () => {
        const 운 = SM.십이운성(R);
    ...
    if (YY) {
      안전(7, '재물운', '재성이 있나, 어디에 앉았나, 누가 낳고 누가 나눠 가나', () => 걸러(YY.재물(R)).map(영칸).join('') + 통설주);
      안전(8, '연애 · 결혼운', '배우자 자리와 배우자별', () => 걸러(YY.연애(R, input.gender === 'F' ? 'F' : 'M')).map(영칸).join('') + 통설주);
      안전(9, '직업운', '격과 가장 많은 글자로 보는 맞는 일', () => 걸러(YY.직업(R, s.격, null, s.상신)).map(영칸).join('') + 통설주);
      안전(10, '건강운', '옛 책이 글자와 몸을 짝지은 법', () => 걸러(YY.건강(R)).map(영칸).join(''));
    }
    ...
    안전(11, '열 해씩 오는 운 — 대운', '열 해마다 천간과 지지에 글자가 하나씩 와요. 천간으로 오면 생각이, 지지로 오면 몸이 움직여요.', () => {
      ...
      return 시주빈말 + 대.map(d => '<div class="jt-du' + (d.지금 ? ' now' : '') + '"><div class="jt-du-h"><b>' + d.시작나이 + ' ~ ' + d.끝나이 + '세</b><span>' + esc(d.간지) + ' · ' + d.시작해 + '년부터</span>' + (d.지금 ? '<i>지금</i>' : '') + '</div>' + 접어(d.줄, d.지금) + '</div>').join('');
    });
