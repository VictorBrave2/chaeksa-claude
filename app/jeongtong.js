...
(function (global) {
  ...
  function 그리기(box, input) {
    ...
    if (YY) {
      out.push(장(7, '재물운', '재성이 있나, 어디에 앉았나, 누가 낳고 누가 나눠 가나', YY.재물(R).map(영칸).join('') + 통설주));
      out.push(장(8, '연애 · 결혼운', '배우자 자리와 배우자별', YY.연애(R, input.gender === 'F' ? 'F' : 'M').map(영칸).join('') + 통설주));
      out.push(장(9, '직업운', '격과 가장 많은 글자로 보는 맞는 일', YY.직업(R, s.격, 통.일, s.상신).map(영칸).join('') + 통설주));
