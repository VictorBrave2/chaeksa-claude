-- 책사 migrate 16~21 한 번에 (2026-09-05). 순서대로 붙여 넣었다. 전부 다시 돌려도 안전하다(create or replace · if not exists · on conflict).
-- Supabase → SQL Editor → New query → 전체 붙여넣기 → Run. 비밀 없음.

-- ════════ migrate-16-visits-returning ════════
